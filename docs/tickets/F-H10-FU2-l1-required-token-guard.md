...
- [ ] **PD2** — Verify Q649 (`después de la siesta piqué queso fresco con membrillo`) is NOT in the OK list. **FAIL 2026-04-29** — Q649 still returns OK CROISSANT. Empirical investigation chain: (1) initial deploy showed Q649 OK with `level1Hit: true`; suspected deploy drift (BUG-DEPLOY-DRIFT-001). (2) Operator clean-cache redeploy of `f70271f` confirmed F-H10-FU2 IS active (`tarta de queso casera` correctly REJECTS, validating Step 2 path). (3) Q649-specific investigation isolated **BUG-H7-P5-OVERSTRIP-001**: F-H10-FU2 Pass 1 correctly rejects via Step 2 (`membrillo` absent), but H7-P5 retry seam at `engineRouter.ts:178` strips `con membrillo` (Cat C trailing strip; pre-con has ≥ 2 tokens), retries with `queso fresco` which has zero HI tokens → falls through to Jaccard-only ACCEPT against CROISSANT. Cross-agent audit verified Option B (re-evaluate retry result against original query) would break ≥ 6 existing F-H7/F-H8 retry tests. **F-H10-FU3 DEFERRED to P3** per user cost-benefit (single FP, narrow pattern, Standard ~3-4h work, better-ROI bugs available). F-H10-FU2 algorithm is verified empirically correct; the persistent Q649 FP is an architectural interaction outside `runCascade` scope, NOT a regression of F-H10-FU2.

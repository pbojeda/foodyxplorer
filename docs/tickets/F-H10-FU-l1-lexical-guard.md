...
- [ ] **AC3 EMPIRICALLY FAILED** — Operator verification 2026-04-28 post-deploy (commit `73e1c97`, api-dev): Q649 STILL returns `OK CROISSANT CON QUESO FRESCO | 343kcal` (battery file `/tmp/qa-dev-post-fH10FU-20260428-1217.txt:649`). Root cause: F-H10-FU spec computed Jaccard against truncated display name `CROISSANT CON QUESO FRESC` (Jaccard=0.20) but actual full name is `CROISSANT CON QUESO FRESCO` (Jaccard=0.50, > threshold 0.25 → guard accepts). Threshold tuning alone cannot fix without breaking legitimate single-token queries. **Filed F-H10-FU2** in `bugs.md` 2026-04-28 entry (algorithm change required: required-token check OR TF-IDF). Leave [ ] permanently — this AC is closed via the follow-up bug, not via this feature.

**Pre-flight Jaccard distribution analysis (executable mechanism)**
- [x] AC4: Tooling extension committed (Phase 0, commit `f41bd13`). Artifact populated 2026-04-28 with empirical analysis from QA battery `/tmp/qa-dev-post-fH10FU-20260428-1217.txt` (650 queries, 136 FTS hits). Threshold 0.25 validated as safe for L1-stripped queries: 0/115 legitimate hits below threshold post-strip. Caveat: 21 hits show raw-query Jaccard < 0.25 — all wrapper-extraction artifacts (L1 sees stripped query with higher Jaccard). Q649 has Jaccard=0.50 (passes guard incorrectly) — separate algorithm issue tracked in F-H10-FU2.
...
| 2026-04-28 | Step 6 — AC3 EMPIRICAL FAIL | Operator deploy api-dev (commit `73e1c97`, ~12:15 UTC). QA battery rerun: 650 queries, OK 435 / NULL 205 / FAIL 10 (vs baseline 435/207/8 = same OK count). **Q649 STILL returns CROISSANT** (`/tmp/qa-dev-post-fH10FU-20260428-1217.txt:649` `OK CROISSANT CON QUESO FRESCO \| 343kcal \| mt=fts_dish`). Root cause: spec computed Jaccard against truncated display `FRESC` (=0.20) but actual full nameEs is `FRESCO` (Jaccard=0.50 > threshold 0.25 → guard accepts). Threshold tuning cannot fix Q649 without breaking legitimate single-token queries (paella, tortilla, gazpacho all = 0.50). Filed F-H10-FU2 in `bugs.md` (algorithm change required). AC3 marked [ ] permanently (closed via follow-up bug, not this feature). |
| 2026-04-28 | Step 6 — AC4 COMPLETE | Pre-flight Jaccard distribution analysis populated at `docs/project_notes/F-H10-FU-jaccard-preflight.md`. 136 FTS hits analyzed (113 fts_dish + 23 fts_food). 115 hits PASS at threshold 0.25; 21 hits show raw-query Jaccard < 0.25 (all wrapper-extraction artifacts — actual L1-stripped queries pass). Threshold 0.25 validated as safe for L1 inputs. Additional false positives surfaced (Q178/Q312 coca cola → Huevas merluza, Q378 oporto → paté, Q345 todo → patatas) — bundled into F-H10-FU2 scope. AC4 marked [x]. |
| 2026-04-28 | Step 6 — drift fix MCE row 1 | Auditor finding: row 1 said `Workflow: 7/8 (Step 6 pending merge)` post-merge. Updated to `Workflow: 8/8 (post-merge, Step 6 [x])` + AC3/AC4 status reflected. |
...
| 1. Mark all items | [x] | AC: 11/13 done + AC4 [x] (operator action complete 2026-04-28) + AC3 [ ] EMPIRICALLY FAILED → F-H10-FU2 follow-up filed; DoD: 12/12; Workflow: 8/8 (post-merge, Step 6 [x]). |

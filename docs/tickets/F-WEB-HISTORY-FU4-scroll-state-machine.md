...
**Status:** Done | **Branch:** bugfix/web-feed-scroll-state-machine (off develop `55528c5`, merged to develop `997f326` via PR #308 squash 2026-06-04)
...
- [x] Memory entry `feedback_layout_effect_phase_swap_needs_full_review` saved 2026-06-04 in this closeout PR. Rule: any `useEffect ↔ useLayoutEffect` swap on scroll/layout-writing effects requires Path B, no Path A. Empirically grounded by FU3→FU4 regression chain (FU3 quick swap on hydration+restore unmasked the FU4 root causes; FU2 deferred MAJOR-2 "smooth auto-no-ops" was unverified).
...
- [x] **Step 7 — Merge + Closeout**: PR #308 squash-merged to develop @ `997f326` 2026-06-04 with owner sign-off post-CI-green. Branch `bugfix/web-feed-scroll-state-machine` deleted local + remote via `gh pr merge --delete-branch`. This closeout PR (Status Done + tracker 6/6 + bugs.md update + methodology memory save). Operator AC24/AC25/AC26 reverify pending post Render auto-deploy to app-dev (release develop→main gate).
...
| 2026-06-04 | Step 7 (merge + closeout) | PR #308 squash-merged to develop @ `997f326` 2026-06-04 post CI green (ci-success SUCCESS, test-web SUCCESS, 791/791). Owner sign-off via gitflow path (no per-branch preview env exists; develop IS the integration environment). Branch `bugfix/web-feed-scroll-state-machine` deleted local + remote via `gh pr merge --delete-branch`. This closeout PR: ticket Status → Done; tracker Features row → done 6/6 + Active Session refreshed to "FU4 merged @ 997f326, operator AC24/25/26 reverify pending"; bugs.md BUG-WEB-FEED-SCROLL-SETTLE-001 → "Code shipped 2026-06-04 commit 997f326 (4-effect state machine + inner-wrapper observer post-C1); operator reverify pending on app-dev"; methodology memory `feedback_layout_effect_phase_swap_needs_full_review` saved. Operator AC24/AC25/AC26 reverify pending post Render auto-deploy to app-dev (gate for bugs.md TRULY FIXED + release develop→main unlock). |

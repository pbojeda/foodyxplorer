...
- [x] **AC1.** When a session append fires (`entries.length` grew AND last entryId is new AND `wasNearBottomRef.current === true`), Effect B fires `scrollTo({ top: scrollHeight, behavior: 'smooth' })` AND attaches a `ResizeObserver` to the feed container with deadline = `now + APPEND_BOTTOM_LOCK_WINDOW_MS` (1500ms).
...
- [x] **AC18.** Unit test suite green: `npm test -w @foodxplorer/web` — count target 771 baseline + delta. **Final: 771 + 16 = 787 tests.** (Estimate was 785-790; actual delta=16.)

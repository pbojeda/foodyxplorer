...
**Status:** Done | **Branch:** feature/F107a-auth-core (deleted post-merge)
...
| 2026-05-15 | Step 5 — post-audit BLOCKERs discovered + fixed | External audit caught 3 BLOCKERs the agent had falsely declared resolved (process failure: lint was run before qa-engineer's commit; `gh pr checks --watch` exit code was read but check statuses were not parsed). **B1 LINT**: 3 unused imports (`beforeAll`, `generateKeyPair`, `SignJWT`) in `f107a.edge-cases.test.ts` → removed. **B2 WEB BUILD**: `getSupabaseBrowserClient()` threw on missing `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY` env vars; AuthProvider invokes it via `useMemo` during render, which fires at static prerender of every page (including `/_not-found`) inheriting `layout.tsx`. CI/Vercel build without env vars → crash. **Fix (option B-derivative)**: `packages/web/src/lib/supabase/browser.ts` now uses placeholder URL+key when env missing, emits browser-side `console.warn` on first use (no server-side noise), returns syntactically-valid client. Auth API calls with placeholder will fail loud at runtime if real values weren't set in Vercel — operator runbook AC24 already documents the env vars. **B3 CI red**: was a downstream symptom of B1+B2; resolved automatically once those pass. All 552 web tests + 4571 api tests still pass after fix; no test relied on the throw behavior. Commit `8435253`. |
| 2026-05-15 | Step 5 — CI green confirmed (empirical) | Post-fix CI run `25921061510` first attempt: 6/7 tests green, `test-shared` failed on flaky time-sensitive boundary test (`webMetrics.schemas.edge-cases.test.ts:174`). Logged as `BUG-DEV-SHARED-WEBMETRICS-BOUNDARY-FLAKE-001` (P3, pre-existing, NOT caused by F107a). Auto-retry via `gh run rerun --failed`: all 6 test jobs SUCCESS, `ci-success` SUCCESS, both Vercel deployments SUCCESS. `gh pr view 279 --json mergeStateStatus` returns **CLEAN** + **MERGEABLE**. Branch protection ruleset `14883955` `ci-success` requirement satisfied. Ready for user merge approval. |
| 2026-05-18 | Step 5 — Re-audit (second external auditor) | User invoked a second external audit cycle on HEAD `c1f308d`. Second auditor re-verified empirical state and **APPROVED**: "mergeStateStatus=CLEAN, ci-success=SUCCESS. Tus fixes en 8435253 abordaron los 3 BLOCKERs correctamente — el placeholder fallback en browser.ts es la opción más limpia de las 3 alternativas planteadas." |
| 2026-05-18 | Step 6 — MERGED to develop | PR #279 squash-merged via `gh pr merge 279 --squash --delete-branch`. Merge commit `b359885` "feat(F107a): Auth core — Supabase Auth + accounts + bearer JWT (web) (#279)" on `develop`. 20 feature commits collapsed to 1. Branch `feature/F107a-auth-core` deleted local + remote. Post-merge sanity check: `npm test` on `develop@b359885` exit 0 (49 web suites / 552 web tests pass; full monorepo green) — zero regressions. Operator action pending (out of repo): operator runs Step 4 Finalize manual smoke checklist in `docs/operations/supabase-auth-setup.md` once Supabase Auth Email provider is configured in dev project + Render env vars set + Vercel `NEXT_PUBLIC_SUPABASE_*` configured. Until then, /login page renders but auth API calls fail loud (placeholder fallback per `8435253`). |

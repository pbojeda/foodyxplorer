...
- [ ] /privacidad, /cookies, /aviso-legal return 200 with GDPR-compliant content
- [ ] og-image.jpg exists in public/ and is 1200x630
- [ ] #waitlist scrolls to WaitlistCTASection, #demo scrolls to ProductDemo
- [ ] Variant D is fully removed from code; `?variant=d` resolves to `'a'`
- [ ] PostSimulatorCTA is hidden until user completes a search
- [ ] `animate-fade-in` class is used (not `animate-fadeIn`)
- [ ] `<html>` has `suppressHydrationWarning`
- [ ] JSON-LD has no SearchAction
- [ ] Canonical URL is set in metadata
- [ ] All existing tests pass (updated for variant D removal)
- [ ] New tests for legal page rendering

...
**Status:** Ready for Merge | **Branch:** bugfix/web-feed-scroll-settle (off develop `172fb23`)
...
- [ ] **Step 6 — Merge + Closeout**: `gh pr create` → CI verify green → owner sign-off → squash-merge to develop → closeout PR (Status Done + branches deleted local+remote + tracker sync). Then prompt operator AC19/AC20/AC21.
...
| 2 | Definition of Done all marked | [x] | 7/7 DoD items in scope are `[x]`. Two op-smoke-gated items (`bugs.md FIXED status` + `release develop→main gate`) explicitly note "post-merge" — they'll be marked after AC19-21 op smokes. |
| 3 | Workflow Checklist Steps 0-5 complete | [x] | 0/1/2/3/4/5 all `[x]`. Step 6 stays `[ ]` pending PR + merge. |
| 4 | Completion Log per-step rows | [x] | 7 rows covering Step 0 (Spec draft + /review-spec + sign-off), Step 2 (Plan draft + /review-plan), Step 3 (TDD impl, gates 761/761), Step 5 (code-review + qa + fix-loop, gates 771/771). |
| 5 | Product Tracker sync (Active Session + Features) | [x] | `product-tracker.md` Active Session shows F-WEB-HISTORY-FU2 Step 5/6 (bumped from Step 0). Features table row `in-progress`. To be flipped to `done` in Step 6 closeout. |
...
| 7 | bugs.md sync | [x] | BUG-WEB-FEED-SCROLL-SETTLE-001 entry stays OPEN until merge + op smoke (will flip to FIXED in Step 6 closeout with commit SHA). BUG-SHIM-UNINSTALL-001 (discovered + fixed in this PR's Step 5 fix-loop) marked FIXED with commit reference. |

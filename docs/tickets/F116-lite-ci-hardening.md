...
| 2026-05-13 | Operator action — review_count ROLLBACK | The 2026-05-12 tightening of `required_approving_review_count: 0 → 1` was reverted same-week after PR #269 (this very chore) became the first PR to hit the new gate and surfaced GitHub's hard-coded "PR author cannot self-approve" rule. With single active contributor, the gate would block ALL PRs indefinitely. Ruleset re-patched via `gh api PUT` to `required_approving_review_count: 0`. `dismiss_stale_reviews_on_push: true` RETAINED as future-proofing for when team grows. Doc (`branch-protection-checklist.md`) + bug entry + key_facts.md all updated in same PR #269 to reflect the rollback rationale. Decision: solo-dev autonomy (incl. agent autonomous merges) prioritized over forced review until team size ≥2. **F116 sub-item (7) Operator Action: STILL COMPLETED** — the original concern (no protection at all) is solved; the residual review-count question is parked in the doc + bug trail. |

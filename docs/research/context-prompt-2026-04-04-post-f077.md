# Context Recovery — nutriXplorer (foodXPlorer) — 2026-04-04 (F077 Complete)

## Project State

- **Branch:** develop (clean working tree)
- **Last commit:** 59bff6c — docs: complete F077 — update tracker, clear active session
- **Previous commits:** ba930dc (F077 merged, PR #69), 36a81f7 (context prompt post-F076)
...
- **develop ahead of main by 19 commits** (F071 + F072 + F073 + F074 + F075 + F076 + F077 not yet in main)
...
**No active work.** F077 complete. Ready for **next feature in Phase A1** (F078-F079).
...
- **Problem solved:** Alcohol provides 7 kcal/g — different from standard Atwater factors (protein=4, carbs=4, fat=9). Spanish tapeo culture (beer, wine, vermouth) requires tracking alcohol content. BEDCA already had the data (nutrient ID 221, tagname ALC) but it was buried in `extra.alcohol_g`.
- **Solution:** Promoted alcohol to a first-class nutrient field (`Decimal(8,2) DEFAULT 0`) across the entire pipeline — DB, schemas, SQL queries, estimation cascade, formatters, API spec.
- **Key changes:**
  - `migration.sql` (NEW): `ALTER TABLE food_nutrients/dish_nutrients ADD COLUMN alcohol DECIMAL(8,2) NOT NULL DEFAULT 0`
  - `schema.prisma`: +alcohol field in FoodNutrient and DishNutrient models
  - `foodNutrient.ts`, `dishNutrient.ts`, `estimate.ts`, `menuEstimation.ts`, `recipeCalculate.ts` (shared schemas): +alcohol field in all Zod schemas
  - `bedcaNutrientMapper.ts`: ALC moved from extra.alcohol_g to `STANDARD_FIELD_MAP['ALC'] = 'alcohol'`. `ALCOHOL_TAGNAME` constant removed.
  - `types.ts` (estimation): +alcohol in DishQueryRow, FoodQueryRow, IngredientNutrientRow, mapNutrients(), mapLevel2RowToResult()
  - `level1Lookup.ts`: +4 SQL SELECT columns (`rdn.alcohol::text` / `rfn.alcohol::text`)
  - `level2Lookup.ts`: +2 SUM aggregation columns
  - `level3Lookup.ts`: +2 SQL SELECT columns
  - `level4Lookup.ts`: +3 SQL SELECT columns + aggregation init/loop
  - `resolveIngredient.ts`: +3 SQL SELECT columns (critical fix found by code review)
  - `portionUtils.ts`: +alcohol in NUMERIC_NUTRIENT_KEYS (now 15 keys)
  - `conversationCore.ts`: +alcohol in NUTRIENT_KEYS + aggregateMenuTotals init
  - `aggregateNutrients.ts`: +alcohol in NUTRIENT_KEYS + KEY_MAP + all zero/null initializers
  - `recipeCalculate.ts` (route): +alcohol in estimateResult nutrients + correctedNutrientRow
  - `estimateFormatter.ts` (bot): `if (n.alcohol > 0)` → `🍺 Alcohol: Xg`
  - `api-spec.yaml`: +alcohol in 4 schema definitions (EstimateNutrients, MenuEstimationTotals, RecipeNutrients, DishNutrient/FoodNutrient) + 23 response examples
  - `kysely-types.ts`, `kysely-enums.ts`: Regenerated from Prisma
- **20th Prisma migration** (`alcohol_nutrient_f077`)
- **No new API endpoints, no new tables** — only a new column on existing tables
- **Reviews:** Production validator (READY, 0 critical), Code review (1 critical fixed: resolveIngredient.ts missing alcohol in 3 SQL queries)
- **19 F077 tests** (10 API unit + 6 shared schema + 3 bot formatter). All pass.
...
| Package | Tests | Notes |
|---------|-------|-------|
| API | 2612 passing (149 files) | Vitest. +10 from F077 (f077.alcohol.unit.test.ts) |
| Bot | 1143 (56 files) | +3 from F077 (f077.alcohol.formatter.test.ts) |
| Shared | 434 | +6 from F077 (f077.alcohol.schemas.test.ts) |
| Scraper | 232 | All pass |
| Landing | 659 | Jest + RTL |
| API E2E | 10 | Real HTTP server (excluded from default run) |
| **Total** | **~5080** | +19 from F077 |
...
- **Pre-existing TS errors** in seedPhaseBedca.ts and recipeCalculate.ts (not F077-related)
...
6. **develop → main sync pending:** F071-F077 merged to develop but not yet in main (19 commits ahead).
...
13. **Comparison formatter missing alcohol (F077):** comparisonFormatter.ts NUTRIENT_ROWS and NUTRIENT_FOCUS_MAP don't include alcohol. Users comparing beers won't see alcohol in comparison table. Low priority — comparison is compact view.
14. **Recipe formatter missing alcohol (F077):** recipeFormatter.ts doesn't show alcohol in totals. For consistency with estimateFormatter (which does show it), could add conditional line. Low priority.
...
| `packages/api/src/estimation/portionUtils.ts` | F077 — NUMERIC_NUTRIENT_KEYS (15 keys) |
| `packages/api/src/ingest/bedca/bedcaNutrientMapper.ts` | F077 — BEDCA ALC → alcohol standard field |
| `packages/api/src/conversation/conversationCore.ts` | F070+F076 — 5-step NL pipeline + menu Step 3.5 |
| `packages/bot/src/formatters/estimateFormatter.ts` | F077 — 🍺 Alcohol display |
| `docs/tickets/F077-alcohol-nutrient-support.md` | F077 ticket — full spec + reviews |
...
1. **develop → main sync pending** — F071-F077 merged to develop but not yet in main (19 commits). Deploy when ready.
2. **Start next Phase A1 feature** — F078 (Regional Aliases, Simple) or F079 (Demand-Driven, Simple). All dependencies met.
...
---
Generated: 2026-04-04 after F077 complete. Phase A1 in progress (F071-F077 done, F078-F079 pending).

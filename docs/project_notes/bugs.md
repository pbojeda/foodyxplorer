...
- **Status**: **TRULY FIXED (pending operator AC5/AC6/AC7 post-merge verification)** — superseded by F-WEB-HISTORY-FU6 architectural rewrite to `react-virtuoso` (2026-06-06). All manual scroll machinery (Effects A/B/C/D, `scrollLockRef`, `hydrationReady` gate) deleted; library-owned scroll handles this class of bug structurally. Operator gates: AC5 (hydration), AC6 (append), AC7 (prepend) on app-dev post-merge.
...
- **Status**: **TRULY FIXED (pending operator AC5/AC6/AC7 post-merge verification)** — superseded by F-WEB-HISTORY-FU6 architectural rewrite to `react-virtuoso` (2026-06-06). `HistoryLoadMoreSentinel` deleted; `startReached` prop wiring replaces IntersectionObserver entirely. `loadMoreInFlightRef` sync guard preserved in `useSearchHistory` hook. Operator gate: AC7 (prepend anchor) on app-dev post-merge.
...
- **Status**: **TRULY FIXED (pending operator AC5/AC6/AC7 post-merge verification)** — superseded by F-WEB-HISTORY-FU6 architectural rewrite to `react-virtuoso` (2026-06-06). The entire 4-effect state machine (`scrollLockRef` discriminated union, `wasNearBottomRef`, `ResizeObserver`, `hydrationReady` gate, overflow-anchor) has been deleted. `react-virtuoso` handles append scroll via `followOutput="smooth"` and in-place resize via imperative `autoscrollToBottom()` (triggered by `isLoading` flip detection in `useEffect`). Operator gates: AC5 (hydration), AC6 (append shimmer→card), AC7 (loadMore prepend anchor) on app-dev post-merge. Release develop→main ON HOLD until operator confirmation.

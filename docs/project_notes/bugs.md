...
- **Status**: **FIXED IN CODE** in branch `bugfix/web-feed-effect-b-prepend-restore` (off develop `36bec7b`). Web 797/797 tests, lint 0, typecheck 0, build clean. **Operator AC24/AC25/AC26 reverify pending** on app-dev post auto-deploy. TRULY FIXED gate: AC24 lands at bottom WITHOUT auto-loadMore loop + AC25 still PASS (FU4 round 2 + IO root + this fix all cooperative) + AC26 manual loadMore preserves anchor.
...
- **Status**: **FIXED IN CODE** in branch `bugfix/web-feed-loadmore-io-root` (off develop `4a8779c`); web 796/796 tests, lint 0, typecheck 0, build clean. **Operator AC24/AC25/AC26 reverify pending** on app-dev post-merge auto-deploy. TRULY FIXED gate: AC24 PASS (hydration lands at bottom, NO auto-loop) + AC25 still PASS (FU4 round 2 fix preserved) + AC26 PASS (manual loadMore preserves anchor).
...
- **Status**: **FIXED in code @ `997f326`** via PR #308 (F-WEB-HISTORY-FU4), 2026-06-04 — after FU2/FU3 partial-fix iterations failed to converge on AC20-A + AC21. **Fix chain**: (FU2 `be7ebcf` 2026-06-02) `ResizeObserver` ref-held lifecycle + `wasNearBottomRef` scroll listener — closed Bug 2 isNearBottom mis-classification but left AC20-A (append shimmer→card truncation) and AC21 (loadMore anchor loss) open. (FU3 `249f745` 2026-06-02) `useEffect→useLayoutEffect` phase swap for hydration + restore — fixed AC19 reload flicker but unmasked the FU2 deferred MAJOR-2 ("smooth auto-no-ops" was unverified) + FU2 capture-vs-restore phase race. (FU4 `997f326` 2026-06-04) **architectural refactor**: 6 effects → unified 4-effect state machine with discriminated union `scrollLockRef` (`idle`/`bottom-lock`/`prepending`); `handleLoadMore` callback captures `scrollHeight`/`scrollTop` synchronously pre-skeleton; `overflow-anchor: none` explicit (eliminates JS-vs-native-anchor race); `firstEntryIdRef`/`lastEntryIdRef` discriminate append vs prepend beyond `entries.length` alone. **Round 2 fix-loop 2026-06-03** (external auditor C1 BLOCKER, cross-model gemini+codex CONFIRMED): observer target redirected from flex-1 `feedRef` scroll container (whose box stays constrained by HablarShell's `h-[100dvh]` parent — per W3C Resize Observer §3.1/§3.4.8 it would never fire on internal `scrollHeight` growth) to inner block-flow `feedContentRef` wrapper that DOES grow with content. AC1b discriminating invariant pins the observer target. Web 791/791 tests, lint 0, typecheck 0, build clean. Research doc + cross-model artifacts: `docs/research/transcript-feed-scroll-architecture-2026-06-03.md` + `/tmp/audit-c1-verification-2026-06-03/`. **Operator post-deploy reverify pending on app-dev** (Render auto-deploy from develop): AC24 (5× reload land at bottom), AC25 (append shimmer→card fully visible, including Slow 3G), AC26 (loadMore preserves anchor). **AC25 is the AC that empirically exercises the round 2 fix** — if it FAIL, escalate to F-WEB-HISTORY-FU5 Playwright e2e as definitive gate. **TRULY FIXED gate**: AC24+AC25+AC26 all PASS → bugs.md status flips to TRULY FIXED + release develop→main unlocks. **Release develop→main remains ON HOLD** until operator confirmation. Methodology lessons saved: `feedback_jsdom_layout_ac_gap` reaffirmed (jsdom never closes layout-race ACs; the FU4 round 2 was caught by external auditor reading the box-model spec, NOT by jsdom unit tests that all passed); `feedback_layout_effect_phase_swap_needs_full_review` saved 2026-06-04 (FU3 quick swap was the proximate trigger of the FU4 root-cause exposure).

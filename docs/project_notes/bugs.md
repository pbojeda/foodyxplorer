...
### 2026-06-09 — BUG-WEB-FU7-HEADER-AND-MOBILE-SCROLL: 3 precision bugs after F-WEB-HISTORY-FU7 — papelera invisible + query truncate + mobile mid-scroll [MEDIUM — FIXED]

- **Issue**: Three follow-up bugs surfaced by F-WEB-HISTORY-FU7 operator AC reverify on `app-dev.nutrixplorer.com` 2026-06-09 (11/14 PASS for FU7 core architecture, 3 precision FAIL). None architectural — all in TranscriptEntry header strip CSS + TranscriptFeed mount-timing scope:
  - **Bug 1 (web Chrome + Safari)**: DeleteEntryButton (papelera) invisible on desktop. The trash icon never appears in the header strip above persisted entries.
  - **Bug 2 (web + móvil, Chrome + Safari)**: Long query echo text truncates with `…` ellipsis to single line instead of wrapping to 2 lines.
  - **Bug 3 (iOS Safari + Chrome móvil)**: Initial scroll position lands mid-screen on `/hablar` load + reload (last entry not visible above composer). Post-search auto-scroll worked correctly — only mount-time scroll failed. Desktop unaffected.
- **Root Causes** (3 distinct, all small-scope):
  - **Bug 1**: `DeleteEntryButton.tsx:105` className used the Tailwind hover-reveal pattern `opacity-60 md:opacity-0 md:group-hover:opacity-100`. The `md:` prefix (≥768px = desktop) overrode the mobile-visible `opacity-60` with `opacity-0` by default, only revealing to 100% on parent `.group` hover. The parent group is the entire header row (TranscriptEntry.tsx:262). In practice the affordance was undiscoverable on web (no hover state visible without precise pointer over the exact row). Mobile correctly showed `opacity-60` but no operator desktop session would discover the action.
  - **Bug 2**: `TranscriptEntry.tsx:322` className was `truncate flex-1 min-w-0`. `truncate` = `overflow:hidden + text-overflow:ellipsis + white-space:nowrap` (single-line ellipsis by definition). Owner expected multi-line wrap with bounded height.
  - **Bug 3**: `TranscriptFeed.tsx:77-82` mount useEffect ran `el.scrollTop = el.scrollHeight` synchronously after React commit. On iOS Safari + Chrome móvil the layout pipeline (NutritionCard content + font swap + lazy images) keeps growing the scroll container post-commit — `scrollHeight` read inside the useEffect was stale (smaller than the final value once layout settled). Result: user landed mid-feed. Desktop layout settled within the effect tick → operator-PASS there.
- **Why not caught**: Bug 1 + 2 are static CSS — jsdom unit tests asserted neutral things (a11y, props, callbacks) but never directly asserted the className string for hover-reveal vs always-visible. Bug 3 is a real-browser timing race — jsdom doesn't model the layout pipeline + font swap + lazy image timing. Per memory `feedback_jsdom_layout_ac_gap`, layout/iOS ACs need operator-empirical reverify. The FU7 ticket flagged AC5 hydration as operator-pending; operator PASS on desktop, FAIL on mobile — this bug is the mobile-only specialization.
- **Solution** (in PR #322 squash @ `5cba621`):
  1. **Bug 1**: drop `md:opacity-0 md:group-hover:opacity-100` from DeleteEntryButton.tsx:105. Keep `opacity-60` as always-visible base. Add `hover:opacity-100`, `focus-visible:opacity-100`, `group-hover:opacity-100` so the affordance is always visible and emphasised on direct hover, keyboard focus, or row hover. `transition-colors` → `transition-all` so opacity transitions are animated.
  2. **Bug 2**: TranscriptEntry.tsx:322 `truncate` → `line-clamp-2 break-words` (cap to 2 lines, ellipsis if longer, wrap unbroken tokens). `min-w-0` preserved for flex-shrink. Tailwind v3.4 ships line-clamp utilities natively — no plugin.
  3. **Bug 3**: TranscriptFeed.tsx:77-89 wrap mount scroll assignment in `requestAnimationFrame` (same defer pattern as the settle effect at line ~95). rAF fires after the next layout opportunity, giving iOS Safari time to compute final scrollHeight. Defensive re-check of `feedRef.current` inside the rAF callback guards against unmount before frame.
- **Prevention**: For CSS hover-reveal patterns on touch-incapable affordances (`md:opacity-0 md:group-hover:opacity-100` etc), require explicit a11y/discoverability test asserting always-visible baseline. For ANY mount-time scroll-write in chat/feed UIs, default to `requestAnimationFrame` wrap (browser layout pipeline ≠ React commit). Add to memory `feedback_hand_rolled_scroll_anti_pattern` REFINED note.
...
### 2026-06-04 — BUG-WEB-HISTORY-HYDRATION-RACE-001: Effect C reads stale scrollHeight due to HablarShell state-derivation lag + IntersectionObserver delivers stale snapshot post-hydration [HIGH — empirically diagnosed via `?debug=scroll` instrumentation logs]

- **Issue**: After PR #310 (BUG-WEB-HISTORY-LOADMORE-IO-ROOT-001) shipped to app-dev, operator AC24 reverify STILL failed: hydration reload triggered an auto-loadMore loop, ending at the top of the scroll with the oldest entry. Cross-model gemini+codex theoretical diagnosis pointed at IO root (which IS deployed and correct per bundle inspection). The actual root cause required empirical real-browser data from `?debug=scroll` logs.
- **Root Cause** (two cooperating bugs):
  - **Bug A — Effect C stale scrollHeight**: `HablarShell.tsx:138-145` uses a `useEffect`-based state derivation that copies `persistedEntries` (from `useSearchHistory`) into its local `entries` state, then passes `entries` to TranscriptFeed. This creates a **one-render lag**: when `useSearchHistory` batches `setPersistedEntries([...older, ...prev])` + `setIsLoadingMore(false)` in the same `.then()/.finally()` chain, the first render commit propagates the `isLoadingMore=false` to TranscriptFeed but HablarShell's `entries` is STILL the old array (its `useEffect` hasn't fired yet). Effect C[isLoadingMore] fires in this render and reads `container.scrollHeight` — which reflects only the OLD entry count, NOT the prepended new ones. delta = currentScrollHeight (2815) - prevScrollHeight (2815) = 0. The guard `if (delta > 0)` skips the restore, AND the code unconditionally resets `scrollLockRef.current = { mode: 'idle' }`. By the time HablarShell's `useEffect` fires `setEntries(merged)` in the next paint cycle, Effect B's prepend branch sees `mode='idle'` (already reset) and does nothing. User stranded at scrollTop≈0. Sentinel still visible → IntersectionObserver re-fires → loop until cursor exhausted.
  - **Bug B — IntersectionObserver stale snapshot at hydration**: IntersectionObserver computes intersection asynchronously and delivers callbacks LATER. If the sentinel's `useEffect` runs and calls `observe()` BEFORE TranscriptFeed's `useLayoutEffect` (Effect B) hydration scroll completes (HablarShell render lag: persistedEntries arrives → sentinel renders + observes → entries propagate → Effect B scrolls), the IO snapshots `isIntersecting:true` at the pre-scroll position. The callback delivered after Effect B's scroll uses the STALE snapshot (boundingClientRect/rootBounds from BEFORE the scroll) and fires `onLoadMore` even though visually the sentinel is now far above viewport. Empirical evidence (2026-06-04 `?debug=scroll` logs): `[scroll-debug 740.0ms] Sentinel IO callback {isIntersecting: true, targetTop: +68}` fired AFTER `[scroll-debug 739.2ms] Effect B hydration AFTER scrollTo {scrollTop: 2291.5}` — target=+68 implies the snapshot was from when scrollTop=0.
- **Why not caught**: jsdom + Jest tests cannot model: (i) IntersectionObserver's async callback delivery with stale snapshot semantics; (ii) HablarShell's useEffect-driven render lag affecting TranscriptFeed's Effect C timing. Both bugs are race conditions that only manifest in real-browser execution. Cross-model gemini+codex on PR #310 missed this because the relevant Effect C math + HablarShell state derivation patterns require seeing both files together with real DOM timing. The diagnostic instrumentation PR #311 (`?debug=scroll`) was the empirical bridge: console logs with sub-millisecond timestamps showed Effect C running with `currentScrollHeight === prevScrollHeight`, proving the lag.
- **Solution** (in PR `bugfix/web-feed-effect-b-prepend-restore`):
  1. **Move scroll restoration from Effect C to Effect B's prepend branch.** Effect B fires when `entries.length` (and first/last entryId) actually changes — which by definition means the new entries are in the DOM and `container.scrollHeight` reflects the post-prepend state. delta is computed correctly, restore lands.
  2. **Effect C now only resets `mode='idle'` on successful restore** (`delta > 0`). If delta=0 (stale read due to lag), the lock stays `prepending` so Effect B can complete the restore on the next render. This is the state machine's resilience invariant — neither effect alone closes the prepend; both can attempt, but whichever has correct delta wins, and `mode='idle'` only flips on success.
  3. **`hydrationReady` gate on the sentinel.** TranscriptFeed adds local state `hydrationReady=false → true` set at the END of Effect B's hydration branch (after `scrollTo` + `startBottomLock`). HistoryLoadMoreSentinel accepts this as a new prop and includes it in its `useEffect` early-return predicate: the IntersectionObserver is NOT attached until the hydration scroll has actually moved scrollTop to bottom. Eliminates the stale-snapshot window entirely.
- **Prevention**: For ANY scroll-state machine that relies on cross-effect ordering, the resilience invariant is: each effect must check the actual DOM state (`scrollHeight`, `scrollTop`) at its run time AND only release the lock on observed success. Never rely on a single signal (here `isLoadingMore`) for state machine transitions when the data lifecycle (entries propagation) has its own independent signal. Additionally: any `IntersectionObserver` on a target whose layout position depends on async layout effects (hydration scroll, etc.) MUST gate its activation on the completion of those effects — observe() captures intersection eagerly, callbacks fire lazily, and the gap is a race condition. Saved as `feedback_intersection_observer_hydration_gate` (TBD post-merge).
...
### 2026-06-04 — BUG-WEB-HISTORY-LOADMORE-IO-ROOT-001: HistoryLoadMoreSentinel IntersectionObserver missing root → auto-load loop on /hablar hydration [HIGH — surfaced AC24 reverify post-F-WEB-HISTORY-FU4 merge]

- **Issue**: After F-WEB-HISTORY-FU4 (`997f326`) shipped with AC25 (append shimmer→card) working, operator AC24 (hydration reload) FAIL on app-dev: `/hablar` reload with persisted history "loads 10 entries, scrolls UP, loads more, scrolls UP, loads more, … ends at the TOP of the scroll showing the FIRST search ever, not the LAST." AC25 PASS (FU4 round 2 inner-wrapper observer works). AC26 unverifiable because all entries auto-loaded by step 1.
- **Root Cause**: `HistoryLoadMoreSentinel` constructs its `IntersectionObserver` with NO `root` option → defaults to the browser viewport. The sentinel lives inside `feedRef` (the `flex-1 overflow-y-auto` scroll container in TranscriptFeed). With root=null, the IO measures the sentinel against the **browser window**, not against the inner scroll container — so as long as the sentinel's DOM node is anywhere inside the visible page, it intersects the viewport regardless of `feedRef.scrollTop`. Even after FU4's hydration `scrollTo({top: scrollHeight, behavior: 'instant'})` correctly scrolls the inner container to the bottom, the sentinel (which is still inside the page DOM) keeps `isIntersecting=true` for the viewport root, fires `onLoadMore`, captures `prevScrollTop=0` baseline, Effect C restores to `prevScrollTop + delta = 0 + delta` which leaves the user scrolled NEAR THE TOP, sentinel still "visible to viewport root", → loop until `hasMoreHistory=false`. This is a preexisting F-WEB-HISTORY bug that lived dormant in FU1/FU2/FU3 (the FU2 hydration bug masked it; FU3 partial fix surfaced AC20-A/AC21 but didn't exercise AC24 reliably; FU4 round 2 fixed AC25 fully, exposing AC24 as the next visible failure).
- **Why not caught**: Every TranscriptFeed test mocks `HistoryLoadMoreSentinel` (`TranscriptFeed.test.tsx:42`, `TranscriptFeed.edge-cases.test.tsx:29`, `TranscriptFeed.fu4-qa.edge-cases.test.tsx:54`, `fWebHistory.edge-cases.test.tsx:37`), so the IntersectionObserver wiring was never exercised. Direct mock-boundary integration gap (see `feedback_mock_boundary_integration_gap`). Cross-model on FU4 + auditor C1 BLOCKER also focused on `TranscriptFeed.tsx`'s `ResizeObserver`, NOT on the sibling sentinel's IO. jsdom global stub for IntersectionObserver is a no-op (does not model `root`, viewport geometry, or initial-delivery callback).
- **Diagnosis**: Cross-model gemini + codex parallel review 2026-06-04 (artifacts `/tmp/audit-ac24-regression-2026-06-04/{gemini,codex}.txt`) CONVERGED ON: (P1, high confidence) IO root missing; (P2, medium-high) sentinel auto-load armed too early during hydration window; (P3, defensive) `useSearchHistory.loadMore` lacks a sync in-flight guard so rapid IO double-fires could compound the problem.
- **Solution**: (1) `HistoryLoadMoreSentinel` accepts a new `feedRef: RefObject<HTMLDivElement>` prop; passes `{ root: feedRef.current, threshold: 0 }` to `IntersectionObserver`. (2) `TranscriptFeed` passes its `feedRef` to the sentinel. (3) `useSearchHistory` adds `loadMoreInFlightRef = useRef(false)` mutated synchronously in `loadMore` to short-circuit duplicate calls before they schedule another `setIsLoadingMore(true)`. (4) Direct regression tests for `HistoryLoadMoreSentinel` (4 specs in `__tests__/components/HistoryLoadMoreSentinel.test.tsx`) that assert IO `root` is the passed feedRef element + 1 spec for in-flight guard in `useSearchHistory.test.ts`. **NO MOCK of the sentinel** in the new test file — pins the regression class structurally.
- **Prevention**: When a component is universally mocked by its consumers (4/4 consumer tests in this case), the component MUST have its own direct test suite covering the wiring it owns. Apply `feedback_mock_boundary_integration_gap` lesson: mock boundaries mutually mask integration defects. Additionally: for IntersectionObserver-based UI, ALWAYS specify `root` explicitly when the target lives inside a custom scroll container — leaving root=null defaults to viewport and yields false-positive intersections.
...
### 2026-06-01 — BUG-WEB-FEED-SCROLL-SETTLE-001: TranscriptFeed scroll-to-bottom races browser layout-settle on reload + isNearBottom mis-classifies on append [HIGH — FIXED in code @ `997f326` (F-WEB-HISTORY-FU4 round 2); operator AC24/AC25/AC26 reverify pending on app-dev]
...
- **Solution**: Made the global limiter **bearer-aware** (bearer-over-API-key precedence, ADR-027 / **ADR-029**). `getRateLimitKeyGenerator`: `account:<sub>` when `req.accountId`, else `apiKey:<keyId>`, else `ip:<ip>` (per-account bucket, stable across IPs/devices). `getRateLimitMax`: flat **`AUTHENTICATED_RATE_LIMIT_MAX = 600`/15min** per bearer account (abuse backstop only; the DAILY tier quota stays enforced by `actorRateLimit`). Helpers kept **pure + sync** (constant cap, NOT a per-request `resolveAccountTier` → zero DB/Redis on the hot path). Admin bearer NOT exempt (abuse limiter must bound it); admin API keys stay exempt via `allowList`; read-only routes NOT exempt. Ordering verified: `@fastify/rate-limit` attaches a ROUTE-LEVEL `onRequest` hook, and Fastify runs `actorResolver`'s GLOBAL `onRequest` hook (sets `request.accountId`) first — so the invariant holds regardless of `registerX` order (keep actorResolver global). Plus frontend: disambiguate the 429 copy in `HablarShell.tsx` — daily quota (carries `error.details.limit`) → "límite diario … vuelve mañana"; transient/abuse (no details) → "Demasiadas peticiones … espera unos minutos". Rejected Option A (tier-based 100/1000 — free=100 re-trips under the read fan-out) and Option C (exempt read-only routes — leaves them unbounded).
- **Why not caught**: The exported helpers `getRateLimitMax`/`getRateLimitKeyGenerator` were unit-tested only for the api-key and IP paths (f026.auth.test.ts); no test exercised a bearer-authenticated request through the global limiter. The limiter is skipped in `NODE_ENV=test`, so integration coverage relied on pure helper tests that never modeled the bearer case.
- **Prevention**: New helper unit tests for the bearer branch (`bug-ratelimit-bearer.unit.test.ts`: key + max + bearer-over-key precedence + same-account-cross-IP + empty-sub fall-through). Treat "authenticated principal (bearer)" as first-class in BOTH limiters, not just `actorRateLimit`.
...
- **Root Cause**: `conversationCore.ts:353-354` called `extractPortionModifier(trimmed)` BEFORE `extractFoodQuery(cleanQuery)`. Post-F-NLP design assumption: extractFoodQuery would strip the wrapper first and hand a clean string to the count extractor. But the pipeline order was inverted. Documented as KNOWN GAP AC22 in F-COUNT's ticket; closed in this feature.
- **Smoke-assertion drift** (pre-existing): pattern 5 in `CONVERSATIONAL_WRAPPER_PATTERNS` only listed bare infinitives (`comer|tomar|beber|...`), not clitic-suffixed forms (`comerme|tomarme|beberme|...`). Spanish speakers use both; AC2/AC3 queries use clitics.
- **Solution**: (1) Swap call order in `conversationCore.ts`: `extractFoodQuery(trimmed)` → `extractPortionModifier(stripped.query)` → `stripContainerResidual(cleanQuery)`. Wrapped in try/catch with `logger.error` + stable tag `F-NLP-CHAIN-ORDERING:fallback-fired` for defense-in-depth. (2) Extend pattern 5 with optional `(?:me)?` clitic suffix. (3) New private helper `stripContainerResidual` + new exported `POST_COUNT_SERVING_PATTERNS` array (drink vessels deliberately excluded — `caña de cerveza` is a food-semantic identifier). Dual-gate guard (`cleanQuery !== stripped.query && portionMultiplier !== 1`) prevents future no-count modifiers from accidentally stripping serving tokens. (4) Observable deviation (audited): `"dos cañas de cerveza"` → L1 query is `"cañas de cerveza"` (was `"cerveza"`), routing to the specific `Caña de cerveza` catalog entity (portionGrams=200) instead of the generic bare cerveza (portionGrams=330). Preserved behavioural invariant: `nameEs` matches `/caña/i`.
- **Prevention**: Tests at three levels pin the fix: 26 unit tests in `f-nlp-chain.entityExtractor.unit.test.ts`, 6 integration tests via `processMessage()` in `f-nlp-chain.conversationCore.integration.test.ts`, 29 edge-case tests in `f-nlp-chain.edge-cases.test.ts` covering catastrophic inputs (empty string, wrapper-only, count-only), post-count false-positive guards (`café con leche` preservation), catalog-discrimination proof for EC-5/AC7. H5-B (implicit multi-item detection for `"paella y vino"` without `menú:` keyword) was split to follow-up ticket `F-MULTI-ITEM-IMPLICIT` (PR4 of pm-sprint2) after `/review-spec` round 1 revealed the existing `detectMenuQuery` only matches explicit menu triggers.
- **Status**: Fixed. PR #202 squash-merged to `develop` at `c7cee4d` on 2026-04-23. Post-merge sanity: 3723/3723 API tests (+61 new `it()` calls), lint 0, build clean.
- **Severity**: P2 (silent portionMultiplier undercount on common wrapped queries; not a crash, monitoring shows no 5xx regression)
- **Found by**: 650-query QA exhaustive battery (Cat 15), 2026-04-22 — queries `"me he bebido dos cañas de cerveza"` + `"acabo de beberme 3 cañas"` returned NULL/under-scaled
- **Feature**: F-NLP-CHAIN-ORDERING | **PR**: #202 | **Merge**: `c7cee4d` | **Follow-up**: F-MULTI-ITEM-IMPLICIT (PR4)

---

### 2026-04-23 — BUG-API-AUDIO-4XX-001: `POST /conversation/audio` returned 500 for malformed multipart requests

- **Issue**: QA battery smokes (`/tmp/qa-dev-2026-04-22.txt:388-390`) flagged three cases on `POST /conversation/audio` returning HTTP 500 instead of a proper 4xx: (a) no body + no `Content-Type` header → 500 expected 400|415, (b) `Content-Type: text/plain` → 500 expected 400|415, (c) `Content-Type: multipart/form-data` without boundary + no `X-API-Key` → 500 expected 401 (but this expectation was itself stale — see Smoke-assertion-drift below). Symptom: monitoring inflated the prod 5xx rate and client adapters (web `/hablar`, Telegram bot) got no actionable `code` in the error envelope.
- **Root Cause**: `packages/api/src/errors/errorHandler.ts` mapped `VALIDATION_ERROR` → 400, `FST_ERR_CTP_EMPTY_JSON_BODY` → 400, `FST_REQ_FILE_TOO_LARGE` → 413, but had NO mapping for `@fastify/multipart` parse errors (`FST_ERR_CTP_INVALID_MEDIA_TYPE`, `FST_INVALID_MULTIPART_CONTENT_TYPE`) or for missing boundary in a multipart stream. These fell through to the default 500. Additionally, `packages/api/src/routes/conversation.ts` began the handler with `for await (const part of request.parts())` — no short-circuit if the Content-Type was wrong or the boundary was missing.
- **Smoke-assertion drift (related, same root cause class as PR #195 H3)**: The battery smoke asserted `missing api key → 401` on `/conversation/audio`. F091 designed this endpoint as **anonymous-OK** (EAA voice accessibility — `actorRateLimit.ts:39,49` buckets `anonymous: { voice: 30 }`), so the 401 expectation contradicted the shipped contract. The stale assertion was corrected in the same PR from `401` to `400|415`.
- **Solution**: (1) Added 3 new `mapError()` branches → 415 `UNSUPPORTED_MEDIA_TYPE`. Framework-level branches use a generic message (they fire on any route, not just audio). The handler-thrown branch keeps the audio-specific "Content-Type must be multipart/form-data". (2) Added Step 0a guard in `conversation.ts` before `request.parts()`: non-multipart CT → `UNSUPPORTED_MEDIA_TYPE`; multipart without non-empty boundary (regex `/;\s*boundary=([^;\s]+)/i`) → `VALIDATION_ERROR`. (3) Updated `api-spec.yaml` with the new 415 response and extended the 400 description; also removed "WAV" from the endpoint description as a pre-existing docs/code mismatch (the code has always rejected `audio/wav` because F091 has no RIFF duration parser). (4) Updated `qa-exhaustive.sh:443` smoke #359 expectation from `401` to `400|415` with an inline comment.
- **Prevention**: Tests at three levels pin this fix: 10 route-integration tests in `f091.audio.route.test.ts` (every 4xx path + regressions for anonymous happy path + invalid-key 401 + duration-out-of-range 400), 3 direct `mapError()` unit tests in `errorHandler.test.ts` for the defensive framework branches (so they do not become silent dead code if parser registration ever changes), and 8 edge-case tests in `BUG-API-AUDIO-4XX-001.edge-cases.test.ts` pinning the budget-vs-CT-guard ordering invariant + uppercase CT + RFC-quoted boundary + zero-byte audio. Future regressions surface as test failures, not production 500s.
- **Status**: Fixed. PR #197 squash-merged to `develop` at `05a973a` on 2026-04-23. Post-merge sanity: 3668/3668 API tests, lint 0, build clean. Render auto-deploy is OFF on `api-dev`/`bot-dev`/`bot-prod` (manual deploy required to validate on dev).
- **Severity**: P2 (wrong status codes polluted monitoring; no data corruption; no downtime)
- **Found by**: 650-query QA exhaustive battery (smokes #357-359), 2026-04-22
- **Feature**: BUG-API-AUDIO-4XX-001 | **PR**: #197 | **Merge**: `05a973a`

---

### 2026-04-22 — BUG-DEV-FLAKE-001: time-flaky assertion in `webMetrics.schemas.edge-cases.test.ts:174` ("24h boundary, equality inclusive")

- **Issue**: Test `WebMetricsSnapshotSchema — sessionStartedAt edge cases > rejects sessionStartedAt exactly at 24h ago boundary (exclusive — should fail)` failed on the CI run triggered by push-to-main after PR #188 merge (release `develop → main`, run `24772321952` at 2026-04-22 10:03:32Z). Assertion `expect(result.success).toBe(true)` received `false`. Rerun of the failed job at 10:22:15Z passed green — confirming non-deterministic flake, not a product bug. Prod smoke tests on the same merged SHA (10 queries covering BUG-PROD-012 / F-NLP / F-MORPH / F-COUNT / F-DRINK) all returned OK; F091 `/health/voice-budget` also 200. Failure blocked `mergeStateStatus: CLEAN` on the merge-back PR #189 (same HEAD SHA `e89c6bd`) until the rerun flipped ci-success=SUCCESS, then PR #189 merged cleanly.
- **Root Cause**: The test at line 174 creates a timestamp `const boundary = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString()` (exactly 24h ago, to-the-millisecond), then calls `WebMetricsSnapshotSchema.safeParse({ ..., sessionStartedAt: boundary })`. The schema validates `ts >= now - 24h` (equality allowed, inclusive boundary). Between the timestamp creation and the schema's `now` re-evaluation, wall-clock advances by ≥1ms on a slow runner, so by the time the schema checks, the generated `boundary` is strictly `< now - 24h` and the schema rejects it. Race condition inherent to time-relative boundary testing: when the test generates the timestamp and the schema reads `now` on two different clock reads, any forward tick flips the equality edge.
- **Solution (not implemented yet)**: Refactor the test to inject a fixed `now` via dependency injection or `vi.useFakeTimers()` — the schema itself likely reads `Date.now()` internally (or accepts an overridable time function). Without source access to patch the schema to accept a deterministic `now`, the pragmatic short-term fix is: wrap both the boundary timestamp computation and the `safeParse` call inside a single expression that captures `now` once (impossible without refactoring the schema), OR widen the equality window in the schema by 100ms (weakens the semantic guarantee — not recommended), OR mark this test with `vi.setSystemTime()` + `vi.useFakeTimers()` to stop the clock for the duration of the assertion. The `vi.useFakeTimers()` approach is cleanest and standard vitest practice for time-dependent schema tests.
- **Prevention**: All future schema tests touching time-relative boundaries (`<`, `<=`, `>=`, `>` against `Date.now()` or `new Date()`) must use `vi.useFakeTimers()` + `vi.setSystemTime(knownDate)` to make the runner deterministic. Grep `packages/shared/src/__tests__` and `packages/api/src/__tests__` for any test that passes a relative timestamp (`Date.now() - N`) to a schema — these are all flake candidates. Estimated cost: ~30 min audit + per-test fake-timers wrap.
- **Status**: Logged (not fixed). Flake is non-deterministic and rare (first occurrence in 3+ months). Rerun unblocks. Low priority: does not affect prod, does not affect developer workflow in normal conditions. Add to sprint #2 or dedicated tech-debt sprint.
...
- **Issue**: After the release merge #136 (`7e56827`) deployed to prod, all `/conversation/message` and `/estimate` queries returned `{"code":"DB_UNAVAILABLE","cause":"relation \"dish_nutrients\" does not exist"}`. The table existed (verified via `pg_tables`), Prisma `SELECT 1` succeeded (`/health?db=true` = 200), but Kysely queries using unqualified table names failed. Initially misdiagnosed as: (1) prepared statement invalidation post-DDL migration, (2) pgbouncer pool state issue, (3) connection_limit misconfiguration.
- **Root Cause**: Supabase's **transaction pooler (port 6543)** executes `DISCARD ALL` between client sessions. This resets `search_path` to empty (`''`). Prisma and Kysely emit queries with unqualified table names (e.g., `SELECT ... FROM dish_nutrients`, not `FROM public.dish_nutrients`). With empty `search_path`, PostgreSQL cannot resolve unqualified names → `relation does not exist`. The session pooler (port 5432) does NOT run `DISCARD ALL` and preserves the role-default `search_path = public, extensions`. **Diagnosis chain**: `docker run --rm postgres:17 psql "$POOLER_URL_6543" -c "SHOW search_path; SELECT COUNT(*) FROM dish_nutrients;"` → `search_path: (empty)` + `ERROR: relation does not exist` vs `"SELECT COUNT(*) FROM public.dish_nutrients"` → `1135` rows (table exists). Same test on port 5432 → `search_path: public, extensions` + `COUNT: 1135`. **Attempted fixes that DID NOT work**: (1) `ALTER ROLE postgres SET search_path TO public, extensions` — `DISCARD ALL` overrides even role-level defaults; (2) `?options=-c%20search_path%3Dpublic` in DATABASE_URL — silently ignored by Supabase's Supavisor pooler.
- **Solution**: Changed `DATABASE_URL` on Render API services from port **6543** (transaction pooler) to port **5432** (session pooler). Session pooler preserves `search_path` across queries within each session. Applied to `nutrixplorer-api-dev` + `nutrixplorer-api-prod` on 2026-04-17. Bot services (`*-bot-dev`, `*-bot-pro`) do not use `DATABASE_URL` directly, no change needed.
- **Prevention**: (1) **CRITICAL operational rule added to `key_facts.md`**: ALL `DATABASE_URL` env vars MUST use session pooler (port 5432), never transaction pooler (port 6543). (2) When scaling beyond session pooler limits (~20-50 concurrent), use Prisma `previewFeatures = ["multiSchema"]` or Supabase Team plan (configurable search_path), NOT port 6543. (3) `DIRECT_URL` (for migrations) currently uses `pooler:5432` (session mode) — functional but should point to `db.<ref>.supabase.co:5432` (true direct, no pooler) for robustness. Documented as tech-debt.
- **Intermittent pattern explained**: queries that hit Redis cache returned success (cached from earlier probes or dev sync). New queries hit DB → failed. As Redis TTL expired, previously-cached queries also started failing. Pattern: "paella works at T+5min, fails at T+30min" = Redis cache hit → expire → DB fail.
- **Why dev worked but prod didn't**: both used port 6543 (transaction pooler) BUT dev had been deployed more recently and Supabase may have had different pooler behavior per project. After the fix (both on 5432), both work consistently.
- **Status**: Resolved 2026-04-17. Prod API queries verified 5/5 post-fix.
- **Feature**: RELEASE-INCIDENT-001 | **Found by**: post-release smoke testing | **Severity**: P1 (2h+ prod degradation, self-healing via cache)

### 2026-04-17 — BUG-PROD-008: Photo upload returns `UNAUTHORIZED` — web `API_KEY` not seeded in prod `api_keys` table

- **Severity**: P2 (photo upload feature broken on prod, text queries unaffected) | **Area**: `packages/api/src/plugins/auth.ts` (F026) + Vercel `API_KEY` env var + prod `api_keys` table
- **Issue**: Uploading a photo in `/hablar` on prod returned "Error de configuración. Contacta con soporte." — the web proxy at `packages/web/src/app/api/analyze/route.ts` reads `process.env.API_KEY` and sends it as `X-API-Key` header to `/analyze/menu`. The API auth middleware hashes the key (`sha256`) and looks it up in `api_keys` table. No matching row → `UNAUTHORIZED`. Text queries (`/conversation/message`) don't require API key (anonymous callers allowed at line 99-100 of `auth.ts`) so they were unaffected.
- **Root Cause**: The `seedApiKey.ts` script only seeds the **Telegram Bot** key (hardcoded name "Telegram Bot"). The web app's `API_KEY` was configured in Vercel (6 days prior, all 3 envs) but its `sha256` hash was **never inserted** into the prod `api_keys` table. Pre-existing issue — not caused by the release merge (the `/analyze/menu` route + auth middleware were unchanged between `bf2b9b5` and `7e56827`). User had not tested photo upload on prod before this session.
- **Solution**: Computed `sha256` of the Vercel `API_KEY` value (extracted via `vercel env pull --environment=production`) → `INSERT INTO api_keys` via Supabase SQL Editor with `key_hash`, `key_prefix='fxp_b825'`, `name='Web App'`, `tier='free'`, `is_active=true`.
- **Post-fix status**: Auth passes (photo upload reaches the API). BUT the API now returns "No he podido identificar el plato" — this is a **separate issue**: either (a) OpenAI API key (`OPENAI_API_KEY` or `OPENAI_CHAT_MODEL`) not configured on Render prod, (b) the vision/analysis logic can't match the uploaded photo to a dish in the DB, or (c) photo quality issue. **Investigation pending** — next session priority.
- **Prevention**: (1) When adding API keys to Vercel web projects, ALSO seed the corresponding hash into the prod `api_keys` table. (2) Consider a `seedWebAppKey.ts` script (parallel to `seedApiKey.ts`) or generalize `seedApiKey.ts` to accept `--name` and `--key` parameters. (3) Add photo upload to the prod smoke test checklist for future releases.
...
- **Issue**: Photo upload in `/hablar` returns "No he podido identificar el plato" but Render logs show `"error":{}` — the actual OpenAI rejection reason is invisible. Response time ~600ms confirms immediate rejection by OpenAI.
- **Root Cause**: OpenAI SDK throws `APIError` subclasses whose custom properties (`status`, `code`, `type`) are non-enumerable. Pino's `JSON.stringify()` skips non-enumerable properties, producing `{}`. All 6 catch blocks in `openaiClient.ts` suffered from this.
- **Solution**: Added `serializeOpenAIError(error)` helper that extracts `message`, `name`, `status`, `code`, `type` into a plain object. Applied to all 6 error logging sites. TDD: 3 tests (RED→GREEN).
- **Prevention**: (1) Always use `serializeOpenAIError()` when logging caught errors from OpenAI SDK calls. (2) `redactSecrets()` strips `sk-proj-*`, `sk-*`, `key_*` patterns from all logged messages — prevents API key leakage even when SDK embeds credentials in error text. (3) Never log raw `{ error }` for third-party SDK errors.
- **Status**: Fixed + hardened with secret redaction. Root cause was a newline in `OPENAI_API_KEY` env var on Render prod. Key rotated 2026-04-17.
...
- **Issue**: Running `npm run seed:standard-portions` produced no output and seeded 0 rows. The script exported functions (`runSeedFromCsv`, `seedFromParsedRows`, etc.) but had NO top-level invocation — `tsx` loaded the module, defined exports, and exited. The companion `generateStandardPortionCsv.ts` had a CLI entrypoint (`isDirectInvocation` guard + self-invoking function) but the seed script did not.
- **Root Cause**: Likely an oversight during F-UX-B implementation — the seed script was written as a library (testable exports) but the CLI runner was never added. The `isDirectInvocation` pattern from the generate script was not copied over.
- **Solution**: Added CLI entrypoint at the bottom of `seedStandardPortionCsv.ts`: `isDirectInvocation` guard + dynamic `import('@prisma/client')` to avoid PrismaClient loading when imported as a library by tests. Pattern mirrors `generateStandardPortionCsv.ts`.
- **Prevention**: When creating paired scripts (generate + seed, import + export), verify BOTH have CLI entrypoints. Add an integration test that runs `npm run seed:standard-portions -- --dry-run` (future enhancement).
- **Status**: Fixed in PR #139 (`chore/post-release-fixes-2026-04-17`). Verified: seeded 160 rows on both prod and dev.
- **Feature**: Seed CLI runner | **Found by**: release seeding step | **Severity**: P3

### 2026-04-16 — BUG-PROD-005: Render excess build minutes — `render.yaml` `buildFilter` not applied because services were created manually, not via Blueprint

- **Severity**: P2 (operational cost, not user-facing, no outage) | **Area**: Render infrastructure (`render.yaml`, dashboard Build Filters) | **Pipeline Issue 5 of 9**
- **Issue**: Render was consuming more build minutes than expected on the paid plan. Every push to `develop` was rebuilding both `nutrixplorer-api-dev` AND `nutrixplorer-bot-dev` regardless of which files changed; every push to `main` was rebuilding both `nutrixplorer-api-prod` AND `nutrixplorer-bot-prod`. Docs-only pushes, cross-package pushes (e.g., web-only or landing-only), and Markdown-only pushes all triggered rebuilds. The repo's `render.yaml` Blueprint defines the correct `buildFilter` blocks per service (paths + ignoredPaths), but the dashboard Build Filters field on each service was empty.
- **Root Cause**: Render's `render.yaml` `buildFilter` config is only applied when a service is created via **Blueprint flow** (Render Dashboard → Blueprints → New Blueprint Instance → select repo). Services created directly via the manual "New Web Service" / "New Background Worker" flow ignore the Blueprint file entirely — their filters live exclusively in the dashboard's per-service Settings → Build & Deploy → Build Filters field. The user had created all 4 services manually (at different times, before `render.yaml` was added or because Blueprint wasn't used), so the config file was documenting intent but not being applied. User confirmed by inspecting the dashboard on 2026-04-16: _"Build Filters empty in Render dashboard → services were created manually, not via Blueprint"_.
- **Solution**: **Option B — manual dashboard config.** Three options were evaluated:
  1. **(a) Blueprint recreation** — delete all 4 services, recreate via Blueprint. Clean, but requires re-entering env vars and reconnecting external resources (Supabase Postgres, Upstash Redis), plus a non-trivial downtime window for prod. Rejected as invasive.
  2. **(b) Manual dashboard config** — keep services, fill Build Filters field on each service's Settings page with paths from `render.yaml`. No downtime, no env var reconfiguration. `render.yaml` remains canonical intent doc. **Chosen.**
  3. **(c) Accept-as-is** — do nothing. Rejected: cost was growing.
  User applied Option B manually on 2026-04-16 via the Render dashboard. For each of the 4 services, the exact paths from `render.yaml` `buildFilter.paths` + `buildFilter.ignoredPaths` were copy-pasted into the dashboard's Included Paths / Ignored Paths fields. Filter mapping:
  - `nutrixplorer-api-dev` (web, develop) + `nutrixplorer-api-prod` (web, main): Included = `packages/api/**`, `packages/shared/**`, `packages/scraper/**`, `package.json`, `package-lock.json` | Ignored = `docs/**`, `packages/bot/**`, `**/*.md`, `**/*.test.ts`
  - `nutrixplorer-bot-dev` (worker, develop) + `nutrixplorer-bot-prod` (worker, main): Included = `packages/bot/**`, `packages/shared/**`, `package.json`, `package-lock.json` | Ignored = `docs/**`, `packages/api/**`, `packages/scraper/**`, `**/*.md`, `**/*.test.ts`
- **Verification**: BUG-PROD-005 PR is itself docs-only (touches only `docs/**` + `.md`). Expected behavior on merge to `develop`: both `nutrixplorer-api-dev` AND `nutrixplorer-bot-dev` should **skip** rebuild (Render Events tab should show "Ignored" or no new deploy event). Post-merge inspection of the Render Events tab for both dev services is the empirical test. Prod services (`*-prod`) filters remain untested on this PR — they'll be empirically verified on the next `develop → main` release merge (expected to correctly rebuild because the release includes API + shared code changes).
- **Prevention**:
  - **When adding a new Render service, ALWAYS use the Blueprint flow (`New Blueprint Instance`) if `render.yaml` exists in the repo root.** Manual service creation silently discards the `buildFilter` config. There is no UI warning about this — Render's assumption is that if you're creating manually, you want dashboard-only config.
  - **`render.yaml` with manually-created services is documentation of intent, not an applied config.** Treat it as a reference the ops team can copy-paste into the dashboard, not a live source of truth. If the dashboard filter is changed, update `render.yaml` in the same commit to keep intent in sync. Both diverging silently is the failure mode this bug exhibits.
  - **Monitor build-minute consumption after any infra change.** The cost signal is slow (monthly billing cycle), so drift is invisible until the bill arrives. Add a lightweight monthly check of Render's usage page to the operational runbook.
  - **Document the per-service filter mapping in `key_facts.md` Infrastructure section** so that a future session can reconstruct the dashboard state from the project docs without needing Render access.
- **Status**: **Fully resolved 2026-04-16.** Option B applied in Render dashboard. Empirical verification complete for dev services: PR #134 squash-merged to `develop` at `856f752` (`2026-04-16T10:04:16Z`); post-merge Render Events tab inspection by user showed `nutrixplorer-api-dev` last event = `Deploy live for 7e7252c (PR #133)` (10:39 Madrid) and `nutrixplorer-bot-dev` last event = `7e7252c (PR #133)` (10:37 Madrid) — **NO new event for `856f752`** on either service → filter suppressed the build as designed (`docs/**` + `**/*.md` Ignored Paths matched the 100% doc diff). Prod services (`nutrixplorer-pro` + `nutrixplorer-bot-pro`) bonus inspection confirmed last event = `bf2b9b5` (PR #127) from 2026-04-15 — stable, no drift. Prod filter verification on real API diff deferred to next `develop → main` release merge (expected to correctly rebuild both prod services because the release includes API + shared code). Tracker-sync close PR: `chore/tracker-sync-bug-prod-005-close`. Ticket Status: Done.
- **Feature**: BUG-PROD-005 | **Found by**: User operational review (Render billing) | **Severity**: P2 (operational cost) | **Pipeline**: Issue 5 of 9 in the 2026-04-12 production issues pipeline | **Resolved by**: PR #134 (docs + dashboard config applied 2026-04-16) + tracker-sync close PR
...
- **Prevention**:
  ...
  - **Split-cycle tickets each need their own explicit Step 6 housekeeping, AND the tracker Active Session panel must be re-synced every time the cycle's terminal ticket changes.** Discovered via two consecutive external audits on 2026-04-15 (BUG-PROD-004-FU1 cycle). When a feature cycle splits into multiple dependent tickets (e.g., `FU1` → `FU1-REVERT` → `FU1-RETRY`), closing the parent ticket and running a "tracker-sync PR A" synchronizes the project-wide `bugs.md` / `key_facts.md` / Follow-up backlog — but does **NOT** automatically close the downstream ticket files (`BUG-PROD-004-FU1-RETRY-delete-deploy-landing.md` in this case) AND does **NOT** automatically update the tracker Active Session panel. Each ticket must have its own explicit Step 6 completion (Status `Done`, all AC/DoD/Workflow/Merge Checklist Evidence boxes marked, Completion Log extended). And the Active Session panel (`**Active Feature:**` + `**Last Updated:**` + `**Last Completed:**` header lines in `product-tracker.md`) must be updated to reflect the most recent cycle close every single time — otherwise a future session reading the tracker from the top will see "FU1-RETRY in Step 5/6 Review" and conclude a non-existent task is active. **Iteration-loop failure mode observed**: audit #1 found the RETRY ticket file was stale → I ran PR #130 to fix the ticket file but left the Active Session panel stale → audit #2 found the Active Session panel still points at the now-Done RETRY ticket. Both failures share the same root cause class: **downstream documentation state does not follow upstream terminal state in split cycles unless explicitly synced**. **Concrete rule (strengthened)**: at the moment a PR closes a split-cycle ticket, the same commit MUST update (a) the ticket file Status → Done + checkboxes + Completion Log, AND (b) the product-tracker.md Active Session panel (`Last Updated`, `Active Feature`, `Last Completed`) to reflect the new terminal state. If doing this inline in the feature PR is not possible (e.g., because the commit is already merged), the follow-up tracker-sync-finalize PR MUST cover both surfaces in a single PR, not in separate PRs. Do not mark a Task completed until BOTH the ticket file AND the tracker Active Session panel reflect the new terminal state. Run `git grep -n "Active Feature" docs/project_notes/product-tracker.md` after every cycle close to verify the panel points at `None` or the actual next cycle.
...
**Status update 2026-05-11 (F116-lite):** state transition `partially mitigated` → **`mitigated (docs + audit)`**. The original framing ("absence of branch protection") was empirically incorrect: this repo uses GitHub Rulesets (not classic Branch Protection), and ruleset id `14883955` (named `develop`) covers BOTH `refs/heads/develop` AND `refs/heads/main` with `ci-success` as the sole required check + `pull_request` PR-only enforcement + `strict_required_status_checks_policy: true` + `bypass_actors: []`. F116-lite ships the audit + tightening guide at `docs/operations/branch-protection-checklist.md`, including a verbatim table of the current ruleset state, the rulesets-vs-branch-protection distinction (so future engineers do not go down the wrong UI path), and recommended tightening for the beta period (`required_approving_review_count`, `dismiss_stale_reviews_on_push`, restrict `allowed_merge_methods` to squash-only, optional CODEOWNERS). The mandatory state (only `ci-success` required) is **already** correct per the `ci-success` rollup block in `.github/workflows/ci.yml` (comment: "the ONLY required check in branch protection"). Operator action pending: user decides which recommended tightening items to adopt and applies them via the GitHub UI; F116-lite ships only the docs, not the configuration change itself.

**Status update 2026-05-12 (post-pm-hardening operator actions):** state transition `mitigated (docs + audit)` → **`fully mitigated (tightening applied)`**. Recommended tightening adopted: `required_approving_review_count: 0 → 1` and `dismiss_stale_reviews_on_push: false → true` applied to ruleset `14883955` via `gh api PUT repos/pbojeda/foodyxplorer/rulesets/14883955` (commit history of this ticket records the exact JSON diff). `allowed_merge_methods` intentionally kept as `[merge, squash, rebase]` (squash-only would break gitflow release PRs — e.g., #267 needed merge-commit). `require_code_owner_review` + per-branch differentiation DEFERRED (no team yet, no need to differentiate develop vs main rules pre-beta). Empirical verification post-PATCH: `gh api GET .../rulesets/14883955` confirms new values. From now on, every PR against `develop` or `main` requires 1 approval before merge, and approvals auto-invalidate on new commits. **BUG-DEV-CI-001 closed.**

**Status update 2026-05-13 (review_count rollback):** state transition partially walked back to **`tightening partial — review_count rolled back, dismiss_stale retained`**. The 2026-05-12 update incorrectly assumed self-approval would work for solo-dev flow. PR #269 (the chore PR documenting this very tightening) became the first PR to hit the new gate and surfaced the constraint: **GitHub hard-codes "PR author cannot approve own PR"**, regardless of ruleset config. With a single active contributor, `required_approving_review_count: 1` blocks ALL PRs indefinitely. Three workarounds were available: (a) admin bypass via `bypass_actors`, (b) service-account reviewer, (c) revert to `0`. Chose (c) for now — preserves agent-autonomous merge workflow (Claude can `gh pr merge` without requiring a human click), revisit (a) or (b) when team grows. `dismiss_stale_reviews_on_push: true` retained (no-op while `review_count=0` but future-proofs). Ruleset patched again via `gh api PUT`. Empirical verification: post-rollback `gh api GET` confirms `review_count: 0`, `dismiss_stale: true`. **BUG-DEV-CI-001 remains closed** — the original concern (no protection on develop) is solved; the residual review-count question is parked in this audit trail until team size justifies re-enabling.

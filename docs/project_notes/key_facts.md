...
- **Migrations**: `packages/api/prisma/migrations/` — apply with `prisma migrate deploy` (not `migrate dev` due to pgvector shadow DB issue). 8 migrations: init_core_tables + schema_enhancements_f001b + dishes_restaurants_f002 + pgvector_indexes_f003 + embedding_updated_at_f019 + trgm_indexes_f025 + api_keys_f026 + query_logs_f029
...
- **Estimate route**: `packages/api/src/routes/estimate.ts` — `GET /estimate` Fastify plugin (operationId: `estimateAllLevels`). Query params: `EstimateQuerySchema` (query required 1-255, chainSlug optional regex, restaurantId optional UUID). Delegates to `runEstimationCascade()` with `level4Lookup` injected (F024). Redis cache (fail-open, 300s TTL, unified key: `fxp:estimate:<normalized>:<chainSlug>:<restaurantId>`). Miss = HTTP 200 with all hit flags false (never 404). Receives Kysely + Prisma. F029: fire-and-forget query logging via `reply.raw.once('finish', ...)` — zero impact on response timing. X-FXP-Source header detection (bot/api). Passes `logger: request.log` for L4 token usage logging
...
- **Catalog routes**: `packages/api/src/routes/catalog.ts` — Fastify plugin registering 4 read-only catalog endpoints. Plugin options: `{ prisma: PrismaClient, db: Kysely<DB> }` (first route receiving both). `GET /restaurants` (paginated, Prisma), `GET /restaurants/:id/dishes` (paginated, Prisma no-search / Kysely trigram search), `GET /dishes/search` (global trigram, Kysely), `GET /chains` (flat aggregated list, Prisma + JS grouping by chainSlug:countryCode). Cache 60s fail-open. Two mappers: `mapPrismaDishRow` (camelCase + Decimal.toNumber) and `mapKyselyDishRow` (snake_case + Number()). `stableKey()` helper for deterministic cache keys. `BooleanStringSchema` for isActive param (avoids z.coerce.boolean bug). GIN trigram indexes on dishes.name and dishes.name_es (migration `20260319140000_trgm_indexes_f025`). 67 tests (34 route + 33 edge cases)
...
- **Bot wiring**: `src/bot.ts` — `buildBot(config, apiClient)` factory. Creates TelegramBot with `polling: false`. Registers 8 `onText` handlers (anchored regex, `@botname` support), `polling_error` handler, `message` catch-all for unknown commands (KNOWN_COMMANDS set check) + plain text routing to NL handler. `wrapHandler` with nested try/catch for sendMessage failures
- **NL handler**: `src/handlers/naturalLanguage.ts` — `extractFoodQuery(text)` pure parser (8 Spanish prefix patterns, case-insensitive, single-pass first-match-wins), chain slug extraction (last ` en ` + `CHAIN_SLUG_REGEX`), article stripping, fallback to original text. `handleNaturalLanguage(text, apiClient)` async handler — `MAX_NL_TEXT_LENGTH = 500`, delegates to `apiClient.estimate()` + `formatEstimate()`, catches only `ApiError` (rethrows unknown for `wrapHandler`)
- **Tests**: 307 tests across 8 files (config×15, apiClient×17, markdownUtils×32, formatters×40, commands×57, bot×17, edge-cases×53, naturalLanguage×76)

...
- **Estimation module**: `packages/api/src/estimation/` — `level1Lookup.ts` (4-strategy cascade: exact dish → FTS dish → exact food → FTS food, Kysely `sql` tagged templates, CTE de-duplication with `ROW_NUMBER() OVER PARTITION BY`), `level2Lookup.ts` (2-strategy cascade: ingredient_dish_exact → ingredient_dish_fts, CTE aggregation SUM(fn.nutrient * di.gram_weight / 100), per_100g filter before ROW_NUMBER, confidence scoring medium/low), `level3Lookup.ts` (2-strategy cascade: similarity_dish scoped → similarity_food global, pgvector `<->` cosine distance, OpenAI embedding at request time via `callOpenAIEmbeddings`, threshold 0.5 hardcoded, fail-graceful on OpenAI errors, `Number.isFinite` guard on embedding values, confidenceLevel always 'low', estimationMethod always 'extrapolation'), `engineRouter.ts` (F023: `runEstimationCascade()` orchestrates L1→L2→L3→L4 cascade, returns `EngineRouterResult { data: EstimateData, levelHit: 1|2|3|4|null }`, `Level4LookupFn` placeholder type for F024 injection, receives raw query + normalizes internally, `openAiApiKey` via DI not config import), `types.ts` (Level1/Level2/Level3 LookupOptions, Level1/Level2/Level3 Result, DishQueryRow, FoodQueryRow, IngredientNutrientRow, DishSimilarityRow, FoodSimilarityRow, mapDishRowToResult, mapFoodRowToResult, mapLevel2RowToResult, parseDecimal, mapNutrients — all 15 nutrients, all mappers include `similarityDistance: null`), `index.ts` (barrel exports all three lookups + engineRouter + types)
- **Estimate route**: `packages/api/src/routes/estimate.ts` — `GET /estimate` Fastify plugin. Query params: `EstimateQuerySchema` (query required 1-255, chainSlug optional regex, restaurantId optional UUID). Delegates to `runEstimationCascade()` (F023 Engine Router). Redis cache (fail-open, 300s TTL, unified key: `fxp:estimate:<normalized>:<chainSlug>:<restaurantId>`). Miss = HTTP 200 with `level1Hit: false, level2Hit: false, level3Hit: false` (never 404). Receives Kysely instance (not PrismaClient). Route handler ~25 lines: parse → cache check → router call → cache write → reply. `levelHit` from router NOT exposed in HTTP response
- **Estimate schemas**: `packages/shared/src/schemas/estimate.ts` — 7 Zod schemas: `EstimateQuerySchema`, `EstimateMatchTypeSchema` (enum: exact_dish/fts_dish/exact_food/fts_food/ingredient_dish_exact/ingredient_dish_fts/similarity_dish/similarity_food), `EstimateSourceSchema`, `EstimateNutrientsSchema` (15 nutrients + referenceBasis), `EstimateResultSchema` (includes `similarityDistance: z.number().min(0).max(2).nullable()`), `EstimateDataSchema` (includes level1Hit + level2Hit + level3Hit), `EstimateResponseSchema`

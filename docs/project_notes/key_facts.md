...
- **Estimation module**: `packages/api/src/estimation/` — `level1Lookup.ts` (4-strategy cascade: exact dish → FTS dish → exact food → FTS food, Kysely `sql` tagged templates, CTE de-duplication with `ROW_NUMBER() OVER PARTITION BY`), `level2Lookup.ts` (2-strategy cascade: ingredient_dish_exact → ingredient_dish_fts, CTE aggregation SUM(fn.nutrient * di.gram_weight / 100), per_100g filter before ROW_NUMBER, confidence scoring medium/low), `types.ts` (Level1/Level2 LookupOptions, Level1/Level2 Result, DishQueryRow, FoodQueryRow, IngredientNutrientRow, mapDishRowToResult, mapFoodRowToResult, mapLevel2RowToResult, parseDecimal, mapNutrients — all 15 nutrients), `index.ts` (barrel). Both lookups exported for direct F023 consumption
- **Estimate route**: `packages/api/src/routes/estimate.ts` — `GET /estimate` Fastify plugin. Query params: `EstimateQuerySchema` (query required 1-255, chainSlug optional regex, restaurantId optional UUID). L1 → L2 fallback cascade. Redis cache (fail-open, 300s TTL, unified key: `fxp:estimate:<normalized>:<chainSlug>:<restaurantId>`). Miss = HTTP 200 with `level1Hit: false, level2Hit: false` (never 404). Receives Kysely instance (not PrismaClient). L1: `confidenceLevel: 'high'`, `estimationMethod: 'official'`. L2: `confidenceLevel: 'medium'/'low'`, `estimationMethod: 'ingredients'`, `source.type: 'estimated'`
- **Estimate schemas**: `packages/shared/src/schemas/estimate.ts` — 7 Zod schemas: `EstimateQuerySchema`, `EstimateMatchTypeSchema` (enum: exact_dish/fts_dish/exact_food/fts_food/ingredient_dish_exact/ingredient_dish_fts), `EstimateSourceSchema`, `EstimateNutrientsSchema` (15 nutrients + referenceBasis), `EstimateResultSchema`, `EstimateDataSchema` (includes level1Hit + level2Hit), `EstimateResponseSchema`

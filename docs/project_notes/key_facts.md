...
- **Estimation module**: `packages/api/src/estimation/` — `level1Lookup.ts` (4-strategy cascade: exact dish → FTS dish → exact food → FTS food, Kysely `sql` tagged templates, CTE de-duplication with `ROW_NUMBER() OVER PARTITION BY`), `types.ts` (Level1LookupOptions, Level1Result, DishQueryRow, FoodQueryRow, mapDishRowToResult, mapFoodRowToResult, parseDecimal, mapNutrients — all 15 nutrients), `index.ts` (barrel). `level1Lookup` exported for direct F023 consumption
- **Estimate route**: `packages/api/src/routes/estimate.ts` — `GET /estimate` Fastify plugin. Query params: `EstimateQuerySchema` (query required 1-255, chainSlug optional regex, restaurantId optional UUID). Redis cache (fail-open, 300s TTL, key: `fxp:estimate:l1:<normalized>:<chainSlug>:<restaurantId>`). Miss = HTTP 200 with `level1Hit: false` (never 404). Receives Kysely instance (not PrismaClient). Hardcodes `confidenceLevel: 'high'`, `estimationMethod: 'official'` (ADR-001)
- **Estimate schemas**: `packages/shared/src/schemas/estimate.ts` — 7 Zod schemas: `EstimateQuerySchema`, `EstimateMatchTypeSchema` (enum: exact_dish/fts_dish/exact_food/fts_food), `EstimateSourceSchema`, `EstimateNutrientsSchema` (15 nutrients + referenceBasis), `EstimateResultSchema`, `EstimateDataSchema`, `EstimateResponseSchema`

...
**Last Updated:** 2026-04-15 (post BUG-PROD-004-FU1 merge)

**Active Feature:** None — BUG-PROD-004-FU1 squash-merged to `develop` at `2f021c1` (PR #123, 2026-04-15). Next candidates: (a) BUG-PROD-004 Follow-up 2 — Vercel production branch investigation (requires user Vercel dashboard access); (b) BUG-PROD-005 Render excess minutes (still pending user approach decision a/b/c); (c) F116 CI + lint cleanup (2-3 sessions, Standard→Complex).
**Last Completed:** BUG-PROD-004-FU1 — delete redundant `deploy-landing.yml` workflow (Simple, infra-bugfix). Squash-merged to `develop` at `2f021c1` (PR #123, 2026-04-15). Same structural fix as BUG-PROD-004 PR #111 (deleted `deploy-web.yml`): custom `vercel pull/build/deploy` workflow duplicated the Vercel GitHub App native integration already handling landing. Zero code diff, CI 10/10 green, Vercel GH App empirically still posting on PR #123 (`Vercel Preview Comments` pass). `key_facts.md` Infrastructure + Hosting (Landing) sections updated inline to drop stale workflow references. 3 commits squashed: workflow delete + docs updates + DoD restructure. `/audit-merge` 11/11 PASS. User opted out of external audit for this Simple tier (infra-only, zero code paths). Self-review verified: `ci.yml` has zero `deploy-landing` references. Empirical discovery about `develop` branch protection: legacy branch-protection API returns 404, but **rulesets are active** and enforce `ci-success` required check + PR-only (learned when a direct push to `develop` was rejected with `GH013`). Updates `F116.g` understanding: branch protection already exists via rulesets; F116.g task is to tighten it (require up-to-date branch, dismiss stale approvals, no bypass) rather than to create it from scratch. BUG-PROD-004 bugs.md Follow-up 1 marked DONE.
...
**Follow-up backlog:**
- **Follow-up 1** (BUG-PROD-004) — delete `.github/workflows/deploy-landing.yml` (same Vercel GitHub App redundancy, same fix: delete). Separate ticket, Simple tier, no cross-model review needed. Context: `bugs.md` (BUG-PROD-004 entry).
- **Follow-up 2** (BUG-PROD-004) — investigate Vercel production URL not auto-promoting from `develop`. User sees "Promote to Production" button in Vercel dashboard. Most likely cause: Production Branch in Vercel project settings = `main` (correct for gitflow) and user was expecting `develop` to auto-promote. Needs dashboard confirmation before any action. Context: `bugs.md` (BUG-PROD-004 entry).

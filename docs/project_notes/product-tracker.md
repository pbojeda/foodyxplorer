...
**Active Feature:** F-UX-A — Size modifier display in NutritionCard + API (Standard). Step 5/6 (Review). PR #109 open. Branch `feature/F-UX-A-size-modifier-display`. Root cause: backend already detected/scaled portion modifiers (F042) but the web NutritionCard ignored `portionMultiplier` entirely and the API dropped pre-multiplier base values. Fix: new `@foodxplorer/shared/portion/portionLabel.ts` helper (consolidated from bot), new optional `baseNutrients`+`basePortionGrams` fields on `EstimateDataSchema` with paired `superRefine`, orchestrator + /estimate route both capture base before scaling, NutritionCard renders amber `PORCIÓN GRANDE` pill + `base: N kcal` subtitle. Cross-model spec review (Codex + Gemini) before implementation. Post-review hardening: M1 dead ternary, M2 pill prefix unification, M3 defensive clone, P1 /estimate route base capture, P2 near-1.0 epsilon tolerance. 22 new tests in shared, 8 in api, 8 in web. 0 regressions across 5332 workspace tests.
...
4. F-UX-A — size modifier display (grande/pequeña) — **IN PROGRESS** (Step 5/6, PR #109 pending merge)

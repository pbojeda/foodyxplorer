...
**Active Feature:** **F-UX-B** — Spanish portion terms (pincho/tapa/media ración/ración). Standard tier, fullstack. Branch `feature/F-UX-B-spanish-portion-terms` at commit `881157b`. **Step 5 Review COMPLETE — Ready for Merge.** Full TDD pipeline: 14 implementation commits (10 backend `5d1bbbb..f14f6cf` + 4 frontend `cdbd279..9b7ec4e`) + 1 review-hardening commit (`d818033`) + ticket finalization (`881157b`). PR #113 open against develop, ALL CI checks green (ci-success + test-{api,bot,scraper,shared,web} + Vercel preview + Vercel Preview Comments — `deploy-preview` no longer exists thanks to BUG-PROD-004). Cross-model spec review (Codex+Gemini) → spec v2.1 with 7 consensus findings + 1 disagreement arbitrated. Cross-model plan review (first review post sdd-devflow v0.16.7 fix — both reviewers cited project context files, fix verified end-to-end) → plan v1.1 with 3 M1 + 1 M2 + 3 M3 + 1 P2. code-review-specialist + qa-engineer post-implementation → 1 M1 (unescaped `~` in MarkdownV2 — Telegram 400 blocker) + 4 M2 (CSV parser RFC 4180, grams=0 guard, `ración para compartir` substring + dead `'tapa'` branch, M3-2 web/bot termDisplay parity via shared helper) + 2 M3 (obsolete migration test cleanup, generator import-time guard) + 1 P2 all fixed inline. **Workspace test totals: shared 23/596, api 190/3270, bot 68/1221 (1198 baseline preserved), web 34/358 — +110 tests, 0 regressions.** Pending: user merge approval → squash-merge → Step 6 → tracker-sync PR.
...
6. F-UX-B — spanish portion terms (pincho/tapa/ración) — **IN PROGRESS** (PR #113, Standard tier fullstack, **step 5/6 Ready for Merge** — 14 TDD commits + review hardening + ticket finalized, CI all green, awaiting merge approval. Full cross-model review trail: spec v2.1 + plan v1.1 + code review + QA. +110 tests across shared/api/bot/web, 0 regressions, bot 1198 baseline preserved.)

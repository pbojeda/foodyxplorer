...
**Active Feature:** None — BUG-PROD-006 merged to develop at `6b117c9`. Next: BUG-PROD-005 (Render excess minutes) or follow-up tickets. Pending user action: smoke test `POST /conversation/message` with 'tapa de croquetas' on staging (`api-dev.nutrixplorer.com`) to confirm portionAssumption now populated.

...
**Active Feature:** **F-UX-B** — Spanish portion terms (pincho/tapa/media ración/ración). Standard tier, fullstack. Branch `feature/F-UX-B-spanish-portion-terms`. **Backend DONE (commits 1–9 squashed across session)**. All 9 commits committed: shared schema (PortionAssumption + formatPortionTermLabel), computeDisplayPieces util, Prisma migration + schema cleanup, seed CSV pipeline, portionAssumption resolver + orchestrator integration, integration tests, bot snapshot baseline, bot formatter per_dish branch, API spec + docs + ADR-020. Tests: API 3262 / Bot 1215 / Shared 588 — all green. **Next step: frontend implementation (web NutritionCard F-UX-B render paths) — awaiting user checkpoint before continuing.**

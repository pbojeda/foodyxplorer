...
**Active Feature:** BUG-AUDIT-C4 — Fix POST empty body → 500 (Simple, Step 1/6 Setup)
...
| BUG-AUDIT-C4 | Fix POST empty body → 500 | backend | in-progress | 1/6 | Simple. POST `/calculate/recipe` and `/conversation/message` return 500 when body is null/invalid JSON. Fastify body parser throws before Zod. Add onError hook or content-type-parser guard |

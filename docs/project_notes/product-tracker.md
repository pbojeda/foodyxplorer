...
**Active Feature:** None — BUG-PROD-007 merged to `develop` at `aab85f0` (PR #120, squash of 12 commits). Next candidates from the pipeline: (a) Follow-up BUG-PROD-004 — delete `deploy-landing.yml` (Simple, ~15 min); (b) BUG-PROD-005 Render excess minutes (pending user decision on approach — Blueprint recreation vs API config vs accept); (c) Backlog items in `bugs.md` (BUG-F042-COMPOSE-SIZE-MODIFIERS).

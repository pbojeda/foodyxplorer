...
**Active Feature:** BUG-PROD-004 — `deploy-web` workflow cleanup (Simple, CI/infra). **Scope pivoted mid-investigation**: the real root cause was not env var resolution but a redundant workflow. The Vercel GitHub App has been deploying previews successfully on Vercel infrastructure the entire time (user confirmed by reading the Vercel dashboard build logs on 2026-04-12 + confirmed the GH App posts preview URL comments on PRs). The custom `deploy-web.yml` ran in parallel, broken, producing a red `deploy-preview` check that was pure noise. **Fix: deleted `.github/workflows/deploy-web.yml` entirely.** PR #111 now contains three commits (the first two are the investigation trail, the third is the delete); squash-merge will collapse them. Pending: push the delete, watch CI (`ci-success` must stay green, `deploy-preview` checks disappear entirely, `Vercel` GH App check stays green). Follow-ups logged: (1) delete `deploy-landing.yml` for the same reason — separate ticket; (2) investigate why Vercel production URL doesn't auto-promote from develop (user sees a "Promote to Production" button — likely a Vercel project Production Branch setting mismatch with gitflow expectations). F-UX-B branch `feature/F-UX-B-spanish-portion-terms` stays paused at `06fc653` and resumes after Issue 4 merges.

...
**Active Feature:** None — BUG-PROD-006 complete, pending Step 5 QA/Review + merge. Branch: `bug/BUG-PROD-006-f085-fux-b-conversation-wiring`. Steps 0-4 done (spec + cross-model review + TDD: 4 commits, 12/12 GREEN, 3270 unit tests). Next: Step 5 (code-review + qa-engineer + merge checklist + user merge approval), then manual smoke tests on `/hablar`.

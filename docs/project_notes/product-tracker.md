...
**Active Feature:** **F-UX-B** — Spanish portion terms (pincho/tapa/media ración/ración). Standard tier, fullstack. Branch `feature/F-UX-B-spanish-portion-terms` rebased on develop at `5159ce1`. Analysis phase complete (cross-model: Explore + Codex + Gemini, Q1–Q7 decisions locked in with user 2026-04-12). **Currently at Step 0 (Spec)** — writing the final Spec section in the ticket incorporating Q1–Q7 verbatim, the 30 priority dishes, copy discipline rules (`~`, `≈`, `aproximadamente` in aria-label), red-flag mitigation, out-of-scope list. Next: cross-model spec review (Codex + Gemini parallel), user approval of spec, then plan phase (ui-ux-designer BEFORE frontend-planner per feedback memory), cross-model plan review, TDD implementation (shared → api → web → bot), bot regression guarantee (1198 tests byte-identical).

...
**Active Feature:** BUG-PROD-007 — comparison + menu paths missing `prisma` and `originalQuery` (Standard, backend). Branch `bugfix/BUG-PROD-007-comparison-menu-wiring` from `develop`. **Current step:** 5/6 — Ready for Merge (awaiting user approval). PR #120 OPEN / MERGEABLE / CLEAN. `ci-success` + `test-api` 3m58s GREEN on PR. 11 commits ahead of `develop`, 26/26 integration tests GREEN (+ 4 GREEN-from-start regression guards: AC9/AC10/AC11 solo-path + AC12 cache-key spy). `/audit-merge` PASS (status "Ready for Merge", AC 13/13, DoD 13/13, Workflow 7/8, Merge Evidence 8/8 with empirical citations, Completion Log covers all 5 executed workflow steps). Two cross-model review rounds arbitrated (spec v1→v3, plan v1→v2 — Gemini APPROVED both, Codex REVISE on both with empirical citations; all 9 findings adopted). Code review APPROVE + 3 NITs (2 addressed inline). QA PASS WITH FOLLOW-UPS + 1 IMPORTANT (AC8 sentinel hardened inline). External user pre-audit APPROVE prior to `/audit-merge`. Next step: user approves PR #120, squash-merge to `develop`. Previous: SDD v0.17.0 upgrade merged to `develop` at `4389e78` (PR #119); customizations restore merged at `764e147` (PR #118); BUG-PROD-006 merged to `develop` at `6b117c9`.

...
**Active Feature:** BUG-PROD-007 — comparison + menu paths missing `prisma` and `originalQuery` (Standard, backend). Branch `bugfix/BUG-PROD-007-comparison-menu-wiring` from `develop`. **Current step:** Spec v2 ready (cross-model reviewed — Gemini APPROVED empirical, Codex REVISE 4 IMPORTANT all addressed). Scope pivoted from 4 new test files to extending the 2 existing `*.conversationCore.integration.test.ts` files + 1 `cache.test.ts` unit assertion, after Codex flagged ADR-021 naming convention conflict. Next step: backend-planner drafts implementation plan. Previous: SDD v0.17.0 upgrade merged to `develop` at `4389e78` (PR #119); customizations restore merged at `764e147` (PR #118); BUG-PROD-006 merged to `develop` at `6b117c9`.

...
**Last Updated:** 2026-04-15 (post 4-quadrant Vercel verification)

**Active Feature:** None — BUG-PROD-004-FU1-REVERT closed as Done after user completed the unblocker (Vercel GitHub App connection to `nutrixplorer` project) and both projects were empirically verified to deploy correctly on both `develop` (preview) and `main` (production). Follow-up 1 is now UNBLOCKED and ready for retry in a dedicated ticket (BUG-PROD-004-FU1-RETRY — delete `deploy-landing.yml` now that it's both redundant AND broken by the Root Directory config change). Follow-up 2 complete.
...
**Follow-up backlog:**
- **Follow-up 1** (BUG-PROD-004) — delete `.github/workflows/deploy-landing.yml`. **UNBLOCKED 2026-04-15 — ready for retry.** Initial attempt in BUG-PROD-004-FU1 (PR #123, merged at `2f021c1`) was reverted same-day via BUG-PROD-004-FU1-REVERT (PR #125, `30ea01f`). User then completed the prerequisite: connected Vercel GitHub App to `pbojedas-projects/nutrixplorer` (Settings → Git → Connect Git Repository, Root Directory `packages/landing`, Production Branch `main`). 4-quadrant empirical verification PR #126 + PR #127 both green. Workflow is now BOTH redundant AND broken (Root Directory change caused a double-path error in `vercel build`). Retry should happen in a dedicated ticket `BUG-PROD-004-FU1-RETRY-delete-deploy-landing.md` after this tracker-sync PR lands. Full post-mortem and verification audit trail in `bugs.md` BUG-PROD-004-FU1-REVERT entry and in `docs/tickets/BUG-PROD-004-FU1-REVERT-restore-deploy-landing.md` Completion Log.

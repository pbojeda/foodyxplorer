...
**Last Updated:** 2026-04-15 (BUG-PROD-004-FU1 REVERT in progress)

**Active Feature:** BUG-PROD-004-FU1-REVERT — P1 hotfix restoring `.github/workflows/deploy-landing.yml` after BUG-PROD-004-FU1 deletion introduced a silent deploy regression for the landing production site. Step `5/6 (Review)`. Branch `bugfix/BUG-PROD-004-FU1-REVERT-restore-deploy-landing`. Discovery (during Task 2 Vercel production branch investigation): the `pbojedas-projects/nutrixplorer` Vercel project is NOT connected to the Vercel GitHub App (unlike `foodyassistance`/web), so `deploy-landing.yml` was the sole production deploy mechanism. Zero user impact: `nutrixplorer.com` continues to serve the 2026-04-11 build throughout the incident window. File restored byte-identically from commit `0490dfc` (3313 bytes, `cmp` verified). Full post-mortem in `bugs.md` 2026-04-15 entry. BUG-PROD-004 Follow-up 1 reopened as BLOCKED on: user must connect Vercel GH App to the `nutrixplorer` project before any future attempt to remove the workflow.
...
**Follow-up backlog:**
- **Follow-up 1** (BUG-PROD-004) — delete `.github/workflows/deploy-landing.yml`. **REOPENED 2026-04-15 as BLOCKED.** Initial attempt in BUG-PROD-004-FU1 (PR #123, merged at `2f021c1`) was reverted same-day via BUG-PROD-004-FU1-REVERT after discovering the `nutrixplorer` Vercel project is NOT connected to the Vercel GitHub App. **Prerequisite before retry**: user must connect Vercel GH App to `pbojedas-projects/nutrixplorer` via Vercel dashboard (Settings → Git → Connect Git Repository → `pbojeda/foodyxplorer`, Root Directory `packages/landing`, Production Branch `main`). After user confirms connection, next step is empirical verification: open a trivial landing-only PR and confirm a `Vercel` status check appears with a URL pointing to `pbojedas-projects/nutrixplorer/...` (not `foodyassistance`). Only then is the deletion safe to retry in a fresh ticket. Full post-mortem in `bugs.md` 2026-04-15 entry.
- **Follow-up 2** (BUG-PROD-004) — investigate Vercel production URL not auto-promoting from `develop`. **Partially investigated 2026-04-15**: user reports (a) no visible "Production Branch" setting on the `foodyassistance` (web) Vercel project, (b) `nutrixplorer` (landing) Vercel project appears NOT Git-connected. Finding (b) triggered discovery of the BUG-PROD-004-FU1 deploy regression (see Follow-up 1 status). Scope now expanded: (i) user action to connect Vercel GH App to `nutrixplorer` project (blocker for Follow-up 1 retry); (ii) clarify where web's Production Branch is configured (possibly absent because Vercel GH App auto-detects `main` by default — needs dashboard screenshot / confirmation); (iii) document final gitflow promotion model for both projects in `key_facts.md`. Context: `bugs.md` (BUG-PROD-004 entry + BUG-PROD-004-FU1-REVERT post-mortem).

...
**Last Updated:** 2026-04-16 (BUG-PROD-005 Option B applied in Render dashboard; empirical verification PR in progress)

**Active Feature:** BUG-PROD-005 — Render excess build minutes (Simple, infra-bugfix, Path A Quick per bug-workflow skill). Pipeline Issue 5 of 9. **Option B chosen** over (a) Blueprint recreation and (c) accept-as-is. User applied the per-service Build Filters manually in the Render dashboard on 2026-04-16 for all 4 services (`nutrixplorer-api-dev` web/develop, `nutrixplorer-api-prod` web/main, `nutrixplorer-bot-dev` worker/develop, `nutrixplorer-bot-prod` worker/main), copy-pasting the paths from `render.yaml` `buildFilter.paths` + `buildFilter.ignoredPaths`. This PR documents the full history (ticket + `bugs.md` + `key_facts.md` + tracker) and **itself serves as the empirical test vector** — being docs-only, it should NOT trigger a rebuild of `nutrixplorer-api-dev` or `nutrixplorer-bot-dev` on merge to `develop`. Prod service filters (`*-prod`) remain untested on this PR and will be exercised on the next `develop → main` release merge. **Branch**: `bugfix/BUG-PROD-005-render-build-filters` from `develop` at `7e7252c`. **Step**: 3-5 (Implement → Review). Ticket file: `docs/tickets/BUG-PROD-005-render-excess-minutes.md`.
...
7. Issue 5 — BUG-PROD-005 Render excess minutes — **Option B applied 2026-04-16** (Build Filters configured manually in Render dashboard for all 4 services after user confirmed empty filter state meant services were created manually, not via Blueprint, so `render.yaml` `buildFilter` was documentation-only). Docs PR `bugfix/BUG-PROD-005-render-build-filters` pending merge — itself serves as empirical test (docs-only → expect no rebuild of `api-dev` + `bot-dev`). Post-verification tracker-sync PR will mark this DONE with final evidence.

...
**Active Feature:** BUG-PROD-004-FU1 — delete redundant `deploy-landing.yml` workflow (Simple tier, infra-bugfix). Step `5/6 (Review)`. Branch `bugfix/BUG-PROD-004-followup-1-delete-deploy-landing`, PR #123 open against `develop`. Commit `1918b5a` deletes the workflow file + updates `key_facts.md` Infrastructure + Hosting (Landing) sections. Same structural pattern as BUG-PROD-004 PR #111 (deleted `deploy-web.yml`). Vercel GitHub App empirically verified still handling landing on PR #123 (`Vercel Preview Comments` = pass, `Vercel` deploy firing). L5 auto-approved Ticket/Commit; user confirms Merge explicitly.

...
**Active Feature:** BUG-PROD-007 — comparison + menu paths missing `prisma` and `originalQuery` (Standard, backend). Branch `bugfix/BUG-PROD-007-comparison-menu-wiring` from `develop`. **Current step:** Implementation complete + code-review APPROVE + QA PASS WITH FOLLOW-UPS. 11 commits ahead of `develop`. 26/26 BUG-PROD-007 integration tests GREEN (+ 4 GREEN-from-start regression guards: AC9/AC10/AC11 solo-path + AC12 cache-key spy). Two cross-model review rounds: spec v1→v3 (Gemini APPROVED / Codex REVISE 4 IMPORTANT), plan v1→v2 (Gemini APPROVED / Codex REVISE 1 CRITICAL + 3 IMPORTANT + 1 SUGGESTION — all addressed). Code review: 3 NITs (2 addressed inline). QA: 1 IMPORTANT (AC8 hardened inline to be a genuine regression guard for the nullEstimateData branch) + 1 NIT (this tracker update). Next step: open PR, await CI green, then user confirmation before /audit-merge. Previous: SDD v0.17.0 upgrade merged to `develop` at `4389e78` (PR #119); customizations restore merged at `764e147` (PR #118); BUG-PROD-006 merged to `develop` at `6b117c9`.

...
| F115 | Tech Debt: Bot Lint Bankruptcy Cleanup | chore | in-progress | 3/6 | **Simple → medium, High priority.** Clean up pre-existing lint errors across the monorepo: 20 in `packages/bot` (3 production `no-non-null-assertion` + 17 tests across 5 files), 5 in `packages/shared` (4 `no-unused-vars` + 1 `no-non-null-assertion` in tests), 2 in `packages/landing` (invalid eslint-disable directives). Remove `\|\| true` bypasses from `ci.yml:160,163,217` and add `Lint shared` step + ESLint/tsconfig path triggers. **Blocks F094 merge.** See `bugs.md` → BUG-DEV-LINT-001. Running MANUALLY (not PM auto). **Scope discovery:** `packages/api` has 100 pre-existing lint errors — deferred to F116. |

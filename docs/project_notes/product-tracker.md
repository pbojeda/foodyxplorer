...
**Active Feature:** BUG-PROD-004-FU1 — delete redundant `deploy-landing.yml` workflow (Simple tier, infra-bugfix). Step `1/6 (Setup)`. Branch `bugfix/BUG-PROD-004-followup-1-delete-deploy-landing`. Same structural pattern as BUG-PROD-004 PR #111 (deleted `deploy-web.yml`). L5 auto-approves Ticket/Commit checkpoints; user confirms Merge explicitly.

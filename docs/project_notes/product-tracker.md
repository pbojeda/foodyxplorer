...
**Active Feature:** **F-UX-B** — Spanish portion terms (pincho/tapa/media ración/ración). Standard tier, fullstack. Branch `feature/F-UX-B-spanish-portion-terms` at commit `8f7f868`. **Spec v2.1 APPROVED 2026-04-13** — user audited v2 with 2 adjustments (GX2 clamp → low-multiplier fall-through at threshold 0.75; `BUG-DEV-GEMINI-CONFIG` stub ticket created) and 3 clarifications (fallbackReason field for Tier 2 telemetry; seed CSV validation order; bot snapshot mechanism with 7 explicit golden queries). All applied in `f04ba45`/`8f7f868`. **Next step: Plan phase** — ui-ux-designer agent (BEFORE frontend-planner per feedback memory) → backend-planner + frontend-planner in parallel → cross-model plan review (Codex + Gemini, **Gemini now with full project context after sdd-devflow v0.16.7 upgrade** — expect it to cite `.gemini/agents/*`, `.gemini/skills/*`, `base-standards.mdc` as evidence the fix landed) → checkpoint aprobación del plan → TDD (shared → api → web → bot). Bot regression guarantee 1198 tests byte-identical via Jest toMatchSnapshot golden files.

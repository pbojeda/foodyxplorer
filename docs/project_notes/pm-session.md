...
**Step 3 (Implement — current):** `backend-developer` (migration `SearchHistory` + read-only `GET /history` cursor + DELETE×2 + persistence hook skip-text_too_long + prune 500/12m + `transcribedText` on /audio + `INVALID_CURSOR`) → `frontend-developer` (HablarShell singleton→feed, TranscriptFeed/Entry, useSearchHistory loose-parse, delete UI, nudge, telemetry). TDD (api Vitest, web Jest+RTL). Autonomous; quality gates enforced.

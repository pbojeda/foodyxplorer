...
**Status:** Draft — pending Step 6 finalization. Empirically validated via `/hablar-v2` prototype (branch `prototype/hablar-v2-in-column-composer` @ `e285711`) tested on real iPhone Safari + web desktop 2026-06-09: all 4 critical scenarios PASS (BUG A scroll-on-settle + BUG B right-clip + iOS keyboard composer-visible + pin-aware scroll). Cross-model `/review-spec` round 1 = REVISE both → round 2 = Gemini APPROVED + Codex REVISE 3 precision (all closed in rev 2.1). Owner-approved Step 0 → Step 1 progression at the ADR checkpoint per memory `feedback_pre_commit_arch_discussion`.

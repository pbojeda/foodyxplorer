...
async function foodSimilaritySearch(
  ...
  // SECURITY: vectorLiteral is constructed from OpenAI embedding array
  // (validated as finite numbers via embedding.every(Number.isFinite) in callOpenAIEmbeddingsOnce).
  // PostgreSQL vector type does not support parameterized input, so sql.raw() is necessary.
  // Format is "[n1,n2,...,n1536]" where each ni is a finite JavaScript number.

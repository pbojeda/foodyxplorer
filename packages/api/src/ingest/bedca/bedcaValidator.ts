...
import type { BedcaFoodWithNutrients, BedcaValidationResult } from './types.js';
 ...
 */
export function validateBedcaSeedData(
  entries: BedcaFoodWithNutrients[],
): BedcaValidationResult {
  const errors: string[] = [];
  ...
  for (const entry of entries) {
    ...
    const hasName =
      (entry.nameEs?.trim() !== '') || (entry.nameEn?.trim() !== '');
    if (!hasName) {
    ...
    for (const n of entry.nutrients) {
      if (typeof n.value === 'number' && n.value < 0) {
        errors.push(
          `${prefix}: negative nutrient value ${n.value} for nutrientId ${n.nutrientId}`,
        );
      }
    }
    ...
    if (caloriesEntry && typeof caloriesEntry.value === 'number' && caloriesEntry.value > 900) {
      errors.push(
        `[WARN] ${prefix}: calories ${caloriesEntry.value} > 900 kcal/100g (high — verify source)`,
      );
    }
    ...
    const coreNutrients = entry.nutrients.filter(
      (n) => CORE_NUTRIENT_IDS.has(n.nutrientId),
    );
    const allCoreNull =
